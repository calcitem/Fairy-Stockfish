...
constexpr unsigned MAX_INDEX = 2 * 24 * 64 * 64; // stm * psq * wksq * bksq =
                                                 // 196608
...
KPKPosition::KPKPosition(unsigned idx)
{
    ...
    psq = make_square(File((idx >> 13) & 0x3),
                      Rank(RANK_7 - ((idx >> 15) & 0x7)));
    ...
    else if (stm == BLACK && (!(attacks_bb<KING>(ksq[BLACK]) &
                                ~(attacks_bb<KING>(ksq[WHITE]) |
                                  pawn_attacks_bb(WHITE, psq))) ||
                              (attacks_bb<KING>(ksq[BLACK]) &
                               ~attacks_bb<KING>(ksq[WHITE]) & psq)))
        result = DRAW;
