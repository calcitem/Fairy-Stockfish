...
// Ai-wok
// A makruk variant where the met is replaced by a super-piece moving as rook,
// knight, or met
...
// Chaturanga
// The actual rules of the game are not known. This reflects the rules as used
// on chess.com. https://en.wikipedia.org/wiki/Chaturanga
...
Variant *grasshopper_variant()
{
    ...
    v->startFen = "rnbqkbnr/gggggggg/pppppppp/8/8/PPPPPPPP/GGGGGGGG/RNBQKBNR w "
                  "KQkq - 0 1";
    v->doubleStep = false;
    return v;
}
...
{
    Variant *v = chess_variant_base()->init();
    v->startFen = "rnbqkbnr/pppppppp/8/1PP2PP1/PPPPPPPP/PPPPPPPP/PPPPPPPP/"
                  "PPPPPPPP w kq - 0 1";
...
{
    Variant *v = chess_variant_base()->init();
    v->startFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 3+3 0 "
                  "1";
    v->checkCounting = true;
    return v;
}
...
Variant *fivecheck_variant()
{
    ...
    v->startFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 5+5 0 "
                  "1";
    v->nnueAlias = "3check";
    return v;
}
...
Variant *pocketknight_variant()
{
    ...
    v->variantTemplate = "bughouse";
    v->pocketSize = 2;
    v->startFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[Nn] w KQkq - 0 "
                  "1";
    v->pieceDrops = true;
    v->capturesToHand = false;
    return v;
...
// Placement/Pre-chess
// A shuffle variant where the players determine the placing of the back rank
// pieces https://www.chessvariants.com/link/placement-chess
...
// Regional chess variant from Myanmar, similar to Makruk but with a setup
// phase. https://en.wikipedia.org/wiki/Sittuyin
...
Variant *seirawan_variant()
{
    ...
    v->startFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[HEhe] w "
                  "KQBCDFGkqbcdfg - 0 1";
    v->gating = true;
...
Variant *dragon_variant()
{
    ...
    v->startFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[Dd] w KQkq - 0 "
                  "1";
    v->pieceDrops = true;
    v->capturesToHand = false;
    v->whiteDropRegion = Rank1BB;
...
Variant *torishogi_variant()
{
    ...
    v->doubleStep = false;
    v->castling = false;
    v->promotedPieceType[SHOGI_PAWN] = CUSTOM_PIECES + 5; // swallow promotes to
                                                          // goose
    v->promotedPieceType[CUSTOM_PIECES] = CUSTOM_PIECES + 6; // falcon promotes
                                                             // to eagle
    v->mandatoryPiecePromotion = true;
    v->dropNoDoubled = SHOGI_PAWN;
...
Variant *euroshogi_variant()
{
    ...
    v->startFen = "1nbgkgn1/1r4b1/pppppppp/8/8/PPPPPPPP/1B4R1/1NGKGBN1[-] w 0 "
                  "1";
    v->promotionRank = RANK_6;
...
Variant *flipello_variant()
{
    ...
    v->startFen = "8/8/8/3pP3/3Pp3/8/8/"
                  "8["
                  "PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPpppppppppppppppppppppppppppp"
                  "pppp] w 0 1";
    v->passOnStalemate = true;
...
Variant *minixiangqi_variant()
{
    ...
    v->mobilityRegion[WHITE][KING] = (Rank1BB | Rank2BB | Rank3BB) &
                                     (FileCBB | FileDBB | FileEBB);
    v->mobilityRegion[BLACK][KING] = (Rank5BB | Rank6BB | Rank7BB) &
                                     (FileCBB | FileDBB | FileEBB);
    v->kingType = WAZIR;
    v->promotionPieceTypes = {};
    v->doubleStep = false;
...
Variant *shogi_variant()
{
    ...
    v->add_piece(LANCE, 'l');
    v->add_piece(SHOGI_KNIGHT, 'n');
    v->startFen = "lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/"
                  "LNSGKGSNL[-] w 0 1";
...
Variant *shoshogi_variant()
{
    ...
    v->startFen = "lnsgkgsnl/1r2e2b1/ppppppppp/9/9/9/PPPPPPPPP/1B2E2R1/"
                  "LNSGKGSNL w 0 1";
    v->capturesToHand = false;
    v->pieceDrops = false;
...
Variant *okisakishogi_variant()
{
    ...
    v->add_piece(QUEEN, 'q');
    v->startFen = "lnsgkqgsnl/1r6b1/pppppppppp/10/10/10/10/PPPPPPPPPP/1B6R1/"
                  "LNSGQKGSNL[-] w 0 1";
...
Variant *capablanca_variant()
{
    ...
    v->add_piece(CHANCELLOR, 'c');
    v->startFen = "rnabqkbcnr/pppppppppp/10/10/10/10/PPPPPPPPPP/RNABQKBCNR w "
                  "KQkq - 0 1";
    v->promotionPieceTypes = {ARCHBISHOP, CHANCELLOR, QUEEN,
                              ROOK,       BISHOP,     KNIGHT};
    return v;
...
{
    Variant *v = capablanca_variant()->init();
    v->startFen = "rnbqckabnr/pppppppppp/10/10/10/10/PPPPPPPPPP/RNBQCKABNR w "
                  "KQkq - 0 1";
    v->nnueAlias = "capablanca";
    return v;
}
...
Variant *janus_variant()
{
    ...
    v->startFen = "rjnbkqbnjr/pppppppppp/10/10/10/10/PPPPPPPPPP/RJNBKQBNJR w "
                  "KQkq - 0 1";
...
Variant *modern_variant()
{
    ...
    v->startFen = "rnbqkmbnr/ppppppppp/9/9/9/9/9/PPPPPPPPP/RNBMKQBNR w KQkq - "
                  "0 1";
    v->promotionPieceTypes = {ARCHBISHOP, QUEEN, ROOK, BISHOP, KNIGHT};
    return v;
}
...
Variant *chancellor_variant()
{
    ...
    v->castlingQueensideFile = FILE_C;
    v->add_piece(CHANCELLOR, 'c');
    v->startFen = "rnbqkcnbr/ppppppppp/9/9/9/9/9/PPPPPPPPP/RNBQKCNBR w KQkq - "
                  "0 1";
...
Variant *embassy_variant()
{
    ...
    v->castlingQueensideFile = FILE_B;
    v->startFen = "rnbqkcabnr/pppppppppp/10/10/10/10/PPPPPPPPPP/RNBQKCABNR w "
                  "KQkq - 0 1";
    v->nnueAlias = "capablanca";
    return v;
}
...
Variant *centaur_variant()
{
    ...
    v->startFen = "rcnbqkbncr/pppppppppp/10/10/10/10/PPPPPPPPPP/RCNBQKBNCR w "
                  "KQkq - 0 1";
...
Variant *courier_variant()
{
    ...
    v->add_piece(COMMONER, 'm');
    v->add_piece(WAZIR, 'w');
    v->startFen = "rnebmk1wbenr/1ppppp1pppp1/6f5/p5p4p/P5P4P/6F5/1PPPPP1PPPP1/"
                  "RNEBMK1WBENR w - - 0 1";
    v->promotionPieceTypes = {FERS};
    v->doubleStep = false;
    v->castling = false;
...
Variant *grand_variant()
{
    ...
    v->add_piece(ARCHBISHOP, 'a');
    v->add_piece(CHANCELLOR, 'c');
    v->startFen = "r8r/1nbqkcabn1/pppppppppp/10/10/10/10/PPPPPPPPPP/1NBQKCABN1/"
                  "R8R w - - 0 1";
    v->promotionPieceTypes = {ARCHBISHOP, CHANCELLOR, QUEEN,
                              ROOK,       BISHOP,     KNIGHT};
    v->promotionRank = RANK_8;
...
Variant *opulent_variant()
{
    ...
    v->startFen = "rw6wr/clbnqknbla/pppppppppp/10/10/10/10/PPPPPPPPPP/"
                  "CLBNQKNBLA/RW6WR w - - 0 1";
...
Variant *tencubed_variant()
{
    ...
    v->maxRank = RANK_10;
    v->maxFile = FILE_J;
    v->startFen = "2cwamwc2/1rnbqkbnr1/pppppppppp/10/10/10/10/PPPPPPPPPP/"
                  "1RNBQKBNR1/2CWAMWC2 w - - 0 1";
    v->add_piece(ARCHBISHOP, 'a');
...
Variant *shako_variant()
{
    ...
    v->add_piece(CANNON, 'c');
    v->startFen = "c8c/ernbqkbnre/pppppppppp/10/10/10/10/PPPPPPPPPP/ERNBQKBNRE/"
                  "C8C w KQkq - 0 1";
...
Variant *clobber10_variant()
{
    ...
    v->startFen = "PpPpPpPpPp/pPpPpPpPpP/PpPpPpPpPp/pPpPpPpPpP/PpPpPpPpPp/"
                  "pPpPpPpPpP/PpPpPpPpPp/pPpPpPpPpP/PpPpPpPpPp/pPpPpPpPpP w 0 "
                  "1";
    return v;
}
// Flipello 10x10
...
Variant *flipello10_variant()
{
    ...
    v->maxRank = RANK_10;
    v->maxFile = FILE_J;
    v->startFen = "10/10/10/10/4pP4/4Pp4/10/10/10/"
                  "10["
                  "PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP"
                  "pppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp"
                  "] w - - 0 1";
...
Variant *amazons_variant()
{
    ...
    v->startFen = "3q2q3/10/10/q8q/10/10/Q8Q/10/10/"
                  "3Q2Q3["
                  "PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPpppppppppppppp"
                  "pppppppppppppppppppppppppppppppp] w - - 0 1";
    v->stalemateValue = -VALUE_MATE;
    v->arrowGating = true;
    return v;
}
...
Variant *xiangqi_variant_base()
{
    ...
    v->add_piece(FERS, 'a');
    v->startFen = "rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR "
                  "w - - 0 1";
    v->mobilityRegion[WHITE][KING] = (Rank1BB | Rank2BB | Rank3BB) &
                                     (FileDBB | FileEBB | FileFBB);
    v->mobilityRegion[BLACK][KING] = (Rank8BB | Rank9BB | Rank10BB) &
                                     (FileDBB | FileEBB | FileFBB);
    v->mobilityRegion[WHITE][FERS] = v->mobilityRegion[WHITE][KING];
    v->mobilityRegion[BLACK][FERS] = v->mobilityRegion[BLACK][KING];
    v->mobilityRegion[WHITE][ELEPHANT] = Rank1BB | Rank2BB | Rank3BB | Rank4BB |
                                         Rank5BB;
    v->mobilityRegion[BLACK][ELEPHANT] = Rank6BB | Rank7BB | Rank8BB | Rank9BB |
                                         Rank10BB;
...
Variant *manchu_variant()
{
    ...
    v->startFen = "rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/9/9/M1BAKAB2 w - "
                  "- 0 1";
    return v;
}
// Supply chess
...
Variant *supply_variant()
{
    Variant *v = xiangqi_variant_base()->init();
    v->variantTemplate = "bughouse";
    v->startFen = "rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/"
                  "RNBAKABNR[] w - - 0 1";
    v->twoBoards = true;
    v->pieceDrops = true;
    v->dropChecks = false;
...
Variant *janggi_variant()
{
    ...
    v->startFen = "rnba1abnr/4k4/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/4K4/"
                  "RNBA1ABNR w - - 0 1";
    ...
    v->diagonalLines = make_bitboard(SQ_D1, SQ_F1, SQ_E2, SQ_D3, SQ_F3, SQ_D8,
                                     SQ_F8, SQ_E9, SQ_D10, SQ_F10);
    v->pass = true;
...
// Modern rules of Janggi, where bikjang is not considered, but material
// counting is. The repetition rules are also adjusted for better compatibility
// with Kakao Janggi.
...
// Casual rules of Janggi, where bikjang and material counting are not
// considered
...
/// VariantMap::init() is called at startup to initialize all predefined
/// variants
...
void VariantMap::init()
{
    ...
    add("fairy", fairy_variant()); // fairy variant used for endgame code
                                   // initialization
...
/// VariantMap::parse_istream reads variants from an INI-style configuration
/// input stream.
...
void VariantMap::parse_istream(std::istream &file)
{
    ...
    while (file.get() &&
           std::getline(std::getline(file, variant, ']'), input)) {
        ...
        while (file.peek() != '[' && std::getline(file, input)) {
            ...
            if (ss.peek() != ';' && ss.peek() != '#') {
                if (DoCheck && !input.empty() &&
                    input.find('=') == std::string::npos)
                    std::cerr << "Invalid sytax: '" << input << "'."
                              << std::endl;
                if (std::getline(std::getline(ss, key, '=') >> std::ws,
                                 value) &&
                    !key.empty())
        ...
        if (variants.find(variant) != variants.end())
            std::cerr << "Variant '" << variant << "' already exists."
                      << std::endl;
        else if (!variant_template.empty() &&
                 variants.find(variant_template) == variants.end())
            std::cerr << "Variant template '" << variant_template
            ...
            Variant *v = !variant_template.empty() ?
                             VariantParser<DoCheck>(attribs).parse(
                                 (new Variant(
                                      *variants.find(variant_template)->second))
                                     ->init()) :
                             VariantParser<DoCheck>(attribs).parse();
            if (v->maxFile <= FILE_MAX && v->maxRank <= RANK_MAX) {
                add(variant, v);
                // In order to allow inheritance, we need to temporarily add
                // configured variants even when only checking them, but we
                // remove them later after parsing is finished.
                if (DoCheck)
