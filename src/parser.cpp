...
bool set(const std::string &value, Bitboard &target)
{
    ...
    while (!ss.eof() && ss >> file && ss >> rank)
        target |= file == '*' ? rank_bb(Rank(rank - 1)) :
                                square_bb(make_square(File(tolower(file) - 'a'),
                                                      Rank(rank - 1)));
    return !ss.fail();
}
    ...
    if (it != config.end()) {
        ...
        if (DoCheck && !valid) {
            std::string typeName = std::is_same<T, int>()   ? "int" :
                                   ...
                                   std::is_same<T, MaterialCounting>() ?
                                                              "MaterialCountin"
                                                              "g" :
                                   std::is_same<T, CountingRule>() ?
                                                              "CountingRule" :
                                   std::is_same<T, ChasingRule>() ?
                                                              "ChasingRule" :
                                   std::is_same<T, EnclosingRule>() ?
                                                              "EnclosingRule" :
                                   std::is_same<T, Bitboard>() ?
                                                              "Bitboard" :
                                                              typeid(T).name();
...
template <bool DoCheck>
void VariantParser<DoCheck>::parse_attribute(const std::string &key,
                                             PieceType &target,
                                             std::string pieceToChar)
{
    ...
    if (it != config.end()) {
        ...
        if (ss >> token &&
            (idx = token == '-' ? 0 : pieceToChar.find(toupper(token))) !=
                std::string::npos)
...
Variant *VariantParser<DoCheck>::parse(Variant *v)
{
        ...
        if (keyValue != config.end() && !keyValue->second.empty()) {
            if (isalpha(keyValue->second.at(0)))
                ...
                if (DoCheck && keyValue->second.at(0) != '-')
                    std::cerr << name
                              << " - Invalid letter: " << keyValue->second.at(0)
                              << std::endl;
                v->remove_piece(pt);
            }
            // betza
            if (is_custom(pt)) {
                if (keyValue->second.size() > 1)
                    v->customPiece[pt - CUSTOM_PIECES] = keyValue->second.substr(
                        2);
                else if (DoCheck)
                    std::cerr << name << " - Missing Betza move notation"
                              << std::endl;
            } else if (pt == KING) {
                if (keyValue->second.size() > 1) {
                    // custom royal piece
    ...
    for (Phase phase : {MG, EG}) {
        const std::string optionName = phase == MG ? "pieceValueMg" :
                                                     "pieceValueEg";
        ...
        if (pv != config.end()) {
            ...
            if (DoCheck && idx == std::string::npos)
                std::cerr << optionName << " - Invalid piece type: " << token
                          << std::endl;
            else if (DoCheck && !ss.eof())
                std::cerr << optionName << " - Invalid piece value for type: "
                          << v->pieceToChar[idx] << std::endl;
        }
    }
    ...
    if (it_prom != config.end()) {
        ...
        while (ss >> token && ((idx = v->pieceToChar.find(toupper(token))) !=
                               std::string::npos))
            v->promotionPieceTypes.insert(PieceType(idx));
        if (DoCheck && idx == std::string::npos && token != '-')
            std::cerr << "promotionPieceTypes - Invalid piece type: " << token
                      << std::endl;
    }
    ...
    if (it_prom_limit != config.end()) {
        ...
        if (DoCheck && idx == std::string::npos)
            std::cerr << "promotionLimit - Invalid piece type: " << token
                      << std::endl;
        else if (DoCheck && !ss.eof())
            std::cerr << "promotionLimit - Invalid piece count for type: "
                      << v->pieceToChar[idx] << std::endl;
    }
    ...
    if (it_prom_pt != config.end()) {
        ...
        if (DoCheck && (idx == std::string::npos || idx2 == std::string::npos))
            std::cerr << "promotedPieceType - Invalid piece type: " << token
                      << std::endl;
    }
    ...
    if (it_ext != config.end()) {
        ...
        while (ss >> token &&
               (idx = token == '*' ? size_t(ALL_PIECES) :
                                     v->pieceToChar.find(toupper(token))) !=
                   std::string::npos)
            v->extinctionPieceTypes.insert(PieceType(idx));
        if (DoCheck && idx == std::string::npos)
            std::cerr << "extinctionPieceTypes - Invalid piece type: " << token
                      << std::endl;
    }
    parse_attribute("extinctionPieceCount", v->extinctionPieceCount);
    parse_attribute("extinctionOpponentPieceCount",
                    v->extinctionOpponentPieceCount);
    ...
    v->conclude(); // In preparation for the consistency checks below, in case
                   // conclude() hasn't been called yet.
    ...
    if (DoCheck) {
        ...
        for (PieceType pt : v->pieceTypes) {
            for (Color c : {WHITE, BLACK})
                if (std::count(v->pieceToChar.begin(), v->pieceToChar.end(),
                    ...
                    std::cerr << piece_name(pt)
                              << " - Ambiguous piece character: "
                              << v->pieceToChar[make_piece(c, pt)] << std::endl;
        }

        // startFen
        if (FEN::validate_fen(v->startFen, v, v->chess960) != FEN::FEN_OK)
            std::cerr << "startFen - Invalid starting position: " << v->startFen
                      << std::endl;

        // pieceToCharTable
        if (v->pieceToCharTable != "-") {
            const std::string fenBoard = v->startFen.substr(
                0, v->startFen.find(' '));
            std::stringstream ss(v->pieceToCharTable);
            char token;
            while (ss >> token)
                if (isalpha(token) &&
                    v->pieceToChar.find(toupper(token)) == std::string::npos)
                    std::cerr
                        << "pieceToCharTable - Invalid piece type: " << token
                        << std::endl;
            for (PieceType pt : v->pieceTypes) {
        ...
        if (!v->checking && v->checkCounting)
            std::cerr << "checkCounting=true requires checking=true."
                      << std::endl;
        if (v->doubleStep && v->doubleStepRankMin > v->doubleStepRank)
            std::cerr << "Inconsistent settings: doubleStepRankMin > "
                         "doubleStepRank."
                      << std::endl;
        if (v->castling && v->castlingRank > v->maxRank)
            std::cerr << "Inconsistent settings: castlingRank > maxRank."
                      << std::endl;
        if (v->castling && v->castlingQueensideFile > v->castlingKingsideFile)
            std::cerr << "Inconsistent settings: castlingQueensideFile > "
        ...
        if (v->pieceTypes.find(KING) != v->pieceTypes.end()) {
            if (v->blastOnCapture)
                std::cerr << "Can not use kings with blastOnCapture."
                          << std::endl;
            if (v->flipEnclosedPieces)
                std::cerr << "Can not use kings with flipEnclosedPieces."
                          << std::endl;
            // We can not fully check support for custom king movements at this
            // point, since custom pieces are only initialized on loading of the
            // variant. We will assume this is valid, but it might cause
            // problems later if it's not.
            if (!is_custom(v->kingType)) {
                ...
                if (pi->hopper[MODALITY_QUIET].size() ||
                    ...
                    std::cerr << piece_name(v->kingType)
                              << " is not supported as kingType." << std::endl;
            }
        }
    }
    return v;
