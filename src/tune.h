...
class Tune
{
    ...
    // Use polymorphism to accomodate Entry of different types in the same
    // vector
    struct EntryBase
    {
    ...
    struct Entry : public EntryBase
    {
        ...
        Entry(const std::string &n, T &v, const SetRange &r)
            ...
            , range(r)
        { }
        void operator=(const Entry &) = delete; // Because 'value' is a
                                                // reference
    ...
    int add(const SetRange &range, std::string &&names, T &value,
            Args &&...args)
    {
        list.push_back(std::unique_ptr<EntryBase>(
            new Entry<T>(next(names), value, range)));
...
public:
    ...
    {
        return instance().add(SetDefaultRange,
                              names.substr(1, names.size() - 2),
