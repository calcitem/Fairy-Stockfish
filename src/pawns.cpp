...
Score evaluate(const Position &pos, Pawns::Entry *e)
{
    ...
    e->blockedCount += popcount(shift<Up>(ourPawns) &
    ...
    while (b) {
        ...
        if ((support | phalanx) && !pos.sittuyin_promotion()) {
            ...
            if (pos.count<PAWN>(Us) > popcount(pos.board_bb()) / 4)
                v = popcount(support | phalanx) *
                    HordeConnected[bool(opposed)][r];
...
Score Entry::evaluate_shelter(const Position &pos, Square ksq) const
{
    ...
    for (File f = File(center - 1); f <= File(center + 1); ++f) {
        ...
        int ourRank = b ? relative_rank(Us, frontmost_sq(Them, b),
                                        pos.max_rank()) :
                          0;

        b = theirPawns & file_bb(f);
        int theirRank = b ? relative_rank(Us, frontmost_sq(Them, b),
                                          pos.max_rank()) :
                            0;
