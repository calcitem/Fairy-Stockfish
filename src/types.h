...
#define pext(b, m) \
    (_pext_u64(b, m) ^ \
     (_pext_u64(b >> 64, m >> 64) << popcount((m << 64) >> 64)))
#else
...
struct Bitboard
{
    ...
    constexpr Bitboard()
        : b64 {0, 0}
    { }
    constexpr Bitboard(uint64_t i)
        : b64 {0, i}
    { }
    constexpr Bitboard(uint64_t hi, uint64_t lo)
        : b64 {hi, lo} {};

    constexpr operator bool() const { return b64[0] || b64[1]; }

    constexpr operator long long unsigned() const { return b64[1]; }

    constexpr operator unsigned() const { return b64[1]; }
    ...
    constexpr Bitboard operator~() const { return Bitboard(~b64[0], ~b64[1]); }
    ...
    inline Bitboard operator*(const Bitboard x) const
    {
        ...
        return Bitboard(b64[0] * x.b64[1] + b64[1] * x.b64[0] + (a_hi * b_hi) +
                            (t1 >> 32) + (t2 >> 32),
...
static_assert(!(PIECE_TYPE_NB & (PIECE_TYPE_NB - 1)), "PIECE_TYPE_NB is not a "
                                                      "power of 2");
...
enum RiderType : int {
    ...
    HOPPING_RIDERS = RIDER_CANNON_H | RIDER_CANNON_V | RIDER_CANNON_DIAG |
                     RIDER_GRASSHOPPER_H | RIDER_GRASSHOPPER_V |
                     RIDER_GRASSHOPPER_D,
...
extern Value EvalPieceValue[PHASE_NB][PIECE_NB];    // variant piece values for
                                                    // evaluation
extern Value CapturePieceValue[PHASE_NB][PIECE_NB]; // variant piece values for
                                                    // captures/search
...
{
    return Piece(pc ^ PIECE_TYPE_NB); // Swap color of piece B_KNIGHT <->
                                      // W_KNIGHT
}
...
{
    return type_of(m) == DROP ? SQ_NONE :
                                Square((m >> SQUARE_BITS) & SQUARE_BIT_MASK);
}
...
inline PieceType promotion_type(Move m)
{
    return type_of(m) == PROMOTION ?
               PieceType((m >> (2 * SQUARE_BITS + MOVE_TYPE_BITS)) &
                         (PIECE_TYPE_NB - 1)) :
               NO_PIECE_TYPE;
}
...
{
    return PieceType((m >> (2 * SQUARE_BITS + MOVE_TYPE_BITS)) &
                     (PIECE_TYPE_NB - 1));
}

inline Square gating_square(Move m)
{
    return Square((m >> (2 * SQUARE_BITS + MOVE_TYPE_BITS + PIECE_TYPE_BITS)) &
                  SQUARE_BIT_MASK);
}
...
{
    return Move((pt << (2 * SQUARE_BITS + MOVE_TYPE_BITS)) + T +
                (from << SQUARE_BITS) + to);
}
...
{
    return Move(
        (pt_in_hand << (2 * SQUARE_BITS + MOVE_TYPE_BITS + PIECE_TYPE_BITS)) +
...
constexpr Move make_gating(Square from, Square to, PieceType pt, Square gate)
{
    return Move((gate << (2 * SQUARE_BITS + MOVE_TYPE_BITS + PIECE_TYPE_BITS)) +
                (pt << (2 * SQUARE_BITS + MOVE_TYPE_BITS)) + T +
                (from << SQUARE_BITS) + to);
}
...
{
    return PieceType(
        (m >> (2 * SQUARE_BITS + MOVE_TYPE_BITS + PIECE_TYPE_BITS)) &
        (PIECE_TYPE_NB - 1));
}
...
inline int dist(Direction d)
{
    return std::abs(d % NORTH) < NORTH / 2 ?
               ...
               std::max(std::abs(d / NORTH) + 1,
                        int(NORTH - std::abs(d % NORTH)));
}
