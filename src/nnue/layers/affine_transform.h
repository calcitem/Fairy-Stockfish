...
public:
    ...
    static constexpr std::size_t SelfBufferSize = ceil_to_multiple(
        OutputDimensions * sizeof(OutputType), CacheLineSize);

    // Size of the forward propagation buffer used from the input layer to this
    // layer
    static constexpr std::size_t BufferSize = PreviousLayer::BufferSize +
                                              SelfBufferSize;
    ...
    bool read_parameters(std::istream &stream)
    {
        ...
        for (std::size_t i = 0; i < OutputDimensions * PaddedInputDimensions;
             ++i)
    ...
    bool write_parameters(std::ostream &stream) const
    {
        ...
        for (std::size_t i = 0; i < OutputDimensions * PaddedInputDimensions;
            ...
            unscrambledWeights[i] =
                weights[(i / 4) % (PaddedInputDimensions / 4) *
                            OutputDimensions * 4 +
                        i / PaddedInputDimensions * 4 + i % 4];
        }
    ...
    propagate(const TransformedFeatureType *transformedFeatures,
              char *buffer) const
    {
        const auto input = previousLayer.propagate(transformedFeatures,
                                                   buffer + SelfBufferSize);
        ...
        [[maybe_unused]] auto m512_add_dpbusd_epi32 =
            [=](__m512i &acc, __m512i a, __m512i b) {
#if defined(USE_VNNI)
        ...
        [[maybe_unused]] auto m512_add_dpbusd_epi32x4 =
            [=](__m512i &acc, __m512i a0, __m512i b0, __m512i a1, __m512i b1,
                ...
                acc = _mm512_add_epi32(acc,
                                       _mm512_add_epi32(product0, product2));
#endif
            };
        ...
        [[maybe_unused]] auto m256_add_dpbusd_epi32 =
            [=](__m256i &acc, __m256i a, __m256i b) {
#if defined(USE_VNNI)
        ...
        [[maybe_unused]] auto m256_add_dpbusd_epi32x4 =
            [=](__m256i &acc, __m256i a0, __m256i b0, __m256i a1, __m256i b1,
                ...
                acc = _mm256_add_epi32(acc,
                                       _mm256_add_epi32(product0, product2));
#endif
            };
        ...
        [[maybe_unused]] auto m128_hadd = [](__m128i sum, int bias) -> int {
            sum = _mm_add_epi32(sum,
                                _mm_shuffle_epi32(sum, 0x4E)); //_MM_PERM_BADC
            sum = _mm_add_epi32(sum,
                                _mm_shuffle_epi32(sum, 0xB1)); //_MM_PERM_CDAB
            return _mm_cvtsi128_si32(sum) + bias;
        };

        [[maybe_unused]] auto m128_add_dpbusd_epi32 =
            [=](__m128i &acc, __m128i a, __m128i b) {
...
#define vec_set_32 _mm_set1_epi32
        ...
        if constexpr (OutputDimensions % OutputSimdWidth == 0) {
            ...
            for (int i = 0; i < (int)NumChunks - 3; i += 4) {
                ...
                for (int j = 0; j * OutputSimdWidth < OutputDimensions; ++j)
                    vec_add_dpbusd_32x4(outptr[j], in0, col0[j], in1, col1[j],
                                        in2, col2[j], in3, col3[j]);
            }
        } else if constexpr (OutputDimensions == 1) {
#if defined(USE_AVX512)
            if constexpr (PaddedInputDimensions % (SimdWidth * 2) != 0) {
                constexpr IndexType NumChunks = PaddedInputDimensions /
                                                SimdWidth;
                const auto inputVector256 = reinterpret_cast<const __m256i *>(
                    input);

                __m256i sum0 = _mm256_setzero_si256();
                const auto row0 = reinterpret_cast<const __m256i *>(
                    &weights[0]);
        ...
        for (IndexType i = 0; i < OutputDimensions; ++i) {
            ...
            __m128i sumHi = Zeros;
            const auto row = reinterpret_cast<const __m128i *>(
                &weights[offset]);
            for (IndexType j = 0; j < NumChunks; ++j) {
                __m128i row_j = _mm_load_si128(&row[j]);
                __m128i input_j = _mm_load_si128(&inputVector[j]);
                __m128i extendedRowLo = _mm_srai_epi16(
                    _mm_unpacklo_epi8(row_j, row_j), 8);
                __m128i extendedRowHi = _mm_srai_epi16(
                    _mm_unpackhi_epi8(row_j, row_j), 8);
                __m128i extendedInputLo = _mm_unpacklo_epi8(input_j, Zeros);
                __m128i extendedInputHi = _mm_unpackhi_epi8(input_j, Zeros);
                __m128i productLo = _mm_madd_epi16(extendedRowLo,
                                                   extendedInputLo);
                __m128i productHi = _mm_madd_epi16(extendedRowHi,
                                                   extendedInputHi);
                sumLo = _mm_add_epi32(sumLo, productLo);
                sumHi = _mm_add_epi32(sumHi, productHi);
            }
            __m128i sum = _mm_add_epi32(sumLo, sumHi);
            __m128i sumHigh_64 = _mm_shuffle_epi32(sum,
                                                   _MM_SHUFFLE(1, 0, 3, 2));
            sum = _mm_add_epi32(sum, sumHigh_64);
            __m128i sum_second_32 = _mm_shufflelo_epi16(sum, _MM_SHUFFLE(1, 0,
                                                                         3, 2));
            ...
            for (IndexType j = 0; j < NumChunks; ++j) {
                ...
                __m64 extendedRowLo = _mm_srai_pi16(
                    _mm_unpacklo_pi8(row_j, row_j), 8);
                __m64 extendedRowHi = _mm_srai_pi16(
                    _mm_unpackhi_pi8(row_j, row_j), 8);
            ...
            const auto row = reinterpret_cast<const int8x8_t *>(
                &weights[offset]);
            for (IndexType j = 0; j < NumChunks; ++j) {
                int16x8_t product = vmull_s8(inputVector[j * 2], row[j * 2]);
                product = vmlal_s8(product, inputVector[j * 2 + 1],
                                   row[j * 2 + 1]);
