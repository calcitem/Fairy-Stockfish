...
Value Endgame<KXK>::operator()(const Position &pos) const
{
    ...
    if (pos.count<QUEEN>(strongSide) || pos.count<ROOK>(strongSide) ||
        ...
        (pos.count<FERS>(strongSide) >= 3 &&
         ...
         (~DarkSquares & pos.pieces(strongSide, FERS))))
        result = std::min(result + VALUE_KNOWN_WIN,
...
Value Endgame<KBNK>::operator()(const Position &pos) const
{
    ...
    Value result = (VALUE_KNOWN_WIN + 3520) + push_close(strongKing, weakKing) +
                   420 *
                       push_to_corner(opposite_colors(strongBishop, SQ_A1) ?
...
Value Endgame<KPK>::operator()(const Position &pos) const
{
    ...
    Square strongKing = normalize(pos, strongSide,
                                  pos.square<KING>(strongSide));
    Square strongPawn = normalize(pos, strongSide,
                                  pos.square<PAWN>(strongSide));
    ...
    if (pos.promotion_rank() != RANK_8 || RANK_MAX != RANK_8 ||
        pos.promotion_piece_types().find(QUEEN) ==
...
Value Endgame<KRKP>::operator()(const Position &pos) const
{
    ...
    Square queeningSquare = make_square(file_of(weakPawn),
                                        relative_rank(weakSide, RANK_8));
    Value result;
...
Value Endgame<KNNKP>::operator()(const Position &pos) const
{
    ...
    Value result = PawnValueEg + 2 * push_to_edge(weakKing, pos) -
...
Value Endgame<KFsPsK>::operator()(const Position &pos) const
{
    ...
    if (pos.count<FERS>(strongSide) >= 3 &&
        ...
        while (b && (!dark || !light)) {
            if (file_of(pop_lsb(b)) % 2 != relative_rank(strongSide,
                                                         pos.promotion_rank(),
                                                         pos.max_rank()) %
                                               2)
                light = true;
...
Value Endgame<KNSK>::operator()(const Position &pos) const
{
    ...
    Value result = VALUE_KNOWN_WIN + push_close(winnerKSq, loserKSq) +
                   push_to_opposing_edge(relative_square(strongSide, loserKSq,
                                                         pos.max_rank()),
                                         pos);
...
{
    assert(verify_material(pos, strongSide,
                           KnightValueMg + SilverValueMg + FersValueMg, 0));
    assert(verify_material(pos, weakSide, RookValueMg, 0));
...
Value Endgame<KSFKF>::operator()(const Position &pos) const
{
    ...
    Value result = SilverValueEg + push_close(winnerKSq, loserKSq) +
...
ScaleFactor Endgame<KBPsK>::operator()(const Position &pos) const
{
    ...
    // If all the pawns are on the same B or G file, then it's potentially a
    // draw
    if ((!(allPawns & ~FileBBB) || !(allPawns & ~FileGBB)) &&
        ...
        if (relative_rank(strongSide, weakPawn) == RANK_7 &&
            ...
            if (relative_rank(strongSide, weakKing) >= RANK_7 &&
                weakKingDist <= 2 && weakKingDist <= strongKingDist)
                return SCALE_FACTOR_DRAW;
        }
    }
...
ScaleFactor Endgame<KRPKR>::operator()(const Position &pos) const
{
    ...
    Square strongKing = normalize(pos, strongSide,
                                  pos.square<KING>(strongSide));
    Square strongRook = normalize(pos, strongSide,
                                  pos.square<ROOK>(strongSide));
    Square strongPawn = normalize(pos, strongSide,
                                  pos.square<PAWN>(strongSide));
    Square weakKing = normalize(pos, strongSide, pos.square<KING>(weakSide));
    ...
    // Pawn on the 7th rank supported by the rook from behind usually wins if
    // the attacking king is closer to the queening square than the defending
    // king, and the defending king cannot gain tempi by threatening the
    // attacking rook.
