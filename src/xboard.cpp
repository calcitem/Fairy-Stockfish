...
void StateMachine::setboard(std::string fen)
{
    ...
    pos.set(variants.find(Options["UCI_Variant"])->second, fen,
            Options["UCI_Chess960"], &states->back(), Threads.main());
}

// do_move() is called when engine needs to apply a move when using XBoard
// protocol.
...
// undo_move() is called when the engine receives the undo command in XBoard
// protocol.
...
std::string StateMachine::highlight(std::string square)
{
    ...
    for (Rank r = pos.max_rank(); r >= RANK_1; --r) {
        for (File f = FILE_A; f <= pos.max_file(); ++f) {
            for (emptyCnt = 0;
                 f <= pos.max_file() &&
                 !((promotions | captures | quiets) & make_square(f, r));
                 ++f)
                ++emptyCnt;
...
void StateMachine::process_command(std::string token, std::istringstream &is)
{
    if (token == "protover") {
        ...
        for (std::string v : variants.get_keys())
            if (v != "chess")
                vars += "," + v;
        sync_cout << "feature setboard=1 usermove=1 time=1 memory=1 smp=1 "
                     "colors=0 draw=0 "
        ...
        if (is >> token) {
            if (Threads.main()->ponder) {
                if (token == UCI::square(pos, from_sq(moveList.back())))
                    ...
                    copy_if(moves.begin(), moves.end(),
                            back_inserter(filteredMoves), [&](const Move m) {
                                return is_ok(from_sq(m)) &&
                                       UCI::square(pos, from_sq(m)) == token;
                            });
                    if (filteredMoves.size()) {
                        static PRNG rng(now());
                        ponderMove = filteredMoves.at(rng.rand<unsigned>() %
                                                      filteredMoves.size());
                    } else
    ...
    } else if (token == "level" || token == "st" || token == "sd" ||
               token == "time" || token == "otim") {
        int num;
        if (token == "level") {
            // moves to go
            is >> limits.movestogo;
            ...
            if (idx != std::string::npos)
                num = std::stoi(token.substr(0, idx)) * 60 +
                      std::stoi(token.substr(idx + 1));
            else
        ...
        // Note: time/otim are in centi-, not milliseconds
            ...
            is >> num;
            Color them = playColor != COLOR_NB ? ~playColor :
                                                 ~pos.side_to_move();
            if (limits.time[them])
        ...
        if (pos.pass()) {
            StateInfo st;
            Position p;
            p.set(pos.variant(), fen, pos.is_chess960(), &st,
                  pos.this_thread());
            Move m;
            std::string passMove = "@@@@";
    ...
    // Bughouse commands
        ...
        // Partner.moveRequested can only be set if search was successfully
        // aborted
        if (moveAfterSearch && Partner.moveRequested) {
            assert(Threads.abort);
            stop();
            sync_cout << "move " << UCI::move(pos, Partner.moveRequested)
                      << sync_endl;
        ...
        if (std::getline(is, token, '[') &&
            std::getline(is, white_holdings, ']') &&
            std::getline(is, token, '[') &&
            std::getline(is, black_holdings, ']')) {
            std::string fen;
            ...
            if (is >> color && is >> pieceType) {
                ...
                fen.insert(fen.find(']'), 1,
                           toupper(color) == 'B' ? tolower(pieceType) :
                                                   toupper(pieceType));
            } else {
