...
void buildPosition(Position &pos, StateListPtr &states, const char *variant,
                   const char *fen, PyObject *moveList, const bool chess960)
{
    states = StateListPtr(new std::deque<StateInfo>(1)); // Drop old and create
                                                         // a new one
    ...
    for (int i = 0; i < numMoves; i++) {
        PyObject *MoveStr = PyUnicode_AsEncodedString(
            PyList_GetItem(moveList, i), "UTF-8", "strict");
...
extern "C" PyObject *pyffish_setOption(PyObject *self, PyObject *args)
{
    ...
    if (Options.count(name)) {
        PyObject *Value = PyUnicode_AsEncodedString(PyObject_Str(valueObj),
                                                    "UTF-8", "strict");
        Options[name] = std::string(PyBytes_AS_STRING(Value));
        Py_XDECREF(Value);
    } else {
        PyErr_SetString(PyExc_ValueError,
                        (std::string("No such option ") + name + "'").c_str());
        return NULL;
    }
    Py_RETURN_NONE;
}
...
extern "C" PyObject *pyffish_startFen(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue(
        "s", variants.find(std::string(variant))->second->startFen.c_str());
}
...
extern "C" PyObject *pyffish_twoBoards(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue(
        "O", variants.find(std::string(variant))->second->twoBoards ? Py_True :
                                                                      Py_False);
}
...
extern "C" PyObject *pyffish_capturesToHand(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue(
        "O", variants.find(std::string(variant))->second->capturesToHand ?
                 Py_True :
                 Py_False);
}
...
extern "C" PyObject *pyffish_getSAN(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "sss|pi", &variant, &fen, &move, &chess960,
                          &notation)) {
        return NULL;
    }
...
extern "C" PyObject *pyffish_getSANmoves(PyObject *self, PyObject *args)
{
    ...
    int chess960 = false;
    Notation notation = NOTATION_DEFAULT;
    if (!PyArg_ParseTuple(args, "ssO!|pi", &variant, &fen, &PyList_Type,
                          &moveList, &chess960, &notation)) {
        return NULL;
    }
    if (notation == NOTATION_DEFAULT)
        notation = default_notation(
    ...
    for (int i = 0; i < numMoves; i++) {
        ...
        if ((m = UCI::to_move(pos, moveStr)) != MOVE_NONE) {
            ...
            PyObject *move = Py_BuildValue(
                "s", SAN::move_to_san(pos, m, notation).c_str());
...
extern "C" PyObject *pyffish_legalMoves(PyObject *self, PyObject *args)
{
    ...
    int chess960 = false;
    if (!PyArg_ParseTuple(args, "ssO!|p", &variant, &fen, &PyList_Type,
                          &moveList, &chess960)) {
        return NULL;
    }
...
extern "C" PyObject *pyffish_getFEN(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ssO!|pppi", &variant, &fen, &PyList_Type,
                          &moveList, &chess960, &sfen, &showPromoted,
                          &countStarted)) {
        return NULL;
    }
    ...
    return Py_BuildValue("s",
                         pos.fen(sfen, showPromoted, countStarted).c_str());
}
...
extern "C" PyObject *pyffish_isOptionalGameEnd(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ssO!|pi", &variant, &fen, &PyList_Type,
                          &moveList, &chess960, &countStarted)) {
        return NULL;
    }
...
// INPUT variant, fen, move list
extern "C" PyObject *pyffish_hasInsufficientMaterial(PyObject *self,
                                                     PyObject *args)
{
    PyObject *moveList;
    Position pos;
...
extern "C" PyObject *pyffish_validateFen(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue(
        "i", FEN::validate_fen(std::string(fen),
                               variants.find(std::string(variant))->second,
                               chess960));
}

static PyMethodDef PyFFishMethods[] = {
    {"version", (PyCFunction)pyffish_version, METH_NOARGS,
     "Get package version."},
    {"info", (PyCFunction)pyffish_info, METH_NOARGS,
     "Get Stockfish version info."},
    {"variants", (PyCFunction)pyffish_variants, METH_NOARGS,
     "Get supported variants."},
    {"set_option", (PyCFunction)pyffish_setOption, METH_VARARGS,
     "Set UCI option."},
    {"load_variant_config", (PyCFunction)pyffish_loadVariantConfig,
     METH_VARARGS, "Load variant configuration."},
    {"start_fen", (PyCFunction)pyffish_startFen, METH_VARARGS,
    ...
    {"captures_to_hand", (PyCFunction)pyffish_capturesToHand, METH_VARARGS,
     "Checks whether the variant rules contains capturesToHand."},
    ...
    {"game_result", (PyCFunction)pyffish_gameResult, METH_VARARGS,
     "Get result from given FEN, considering variant end, checkmate, and "
     "stalemate."},
    {"is_immediate_game_end", (PyCFunction)pyffish_isImmediateGameEnd,
     METH_VARARGS, "Get result from given FEN if variant rules ends the game."},
    {"is_optional_game_end", (PyCFunction)pyffish_isOptionalGameEnd,
     METH_VARARGS,
     "Get result from given FEN it rules enable game end by player."},
    {"has_insufficient_material", (PyCFunction)pyffish_hasInsufficientMaterial,
     METH_VARARGS, "Checks for insufficient material."},
    {"validate_fen", (PyCFunction)pyffish_validateFen, METH_VARARGS,
     "Validate an input FEN."},
    {NULL, NULL, 0, NULL}, // sentinel
};

static PyModuleDef pyffishmodule = {
    PyModuleDef_HEAD_INIT, "pyffish", "Fairy-Stockfish extension module.", -1,
    PyFFishMethods,
};
...
PyMODINIT_FUNC PyInit_pyffish()
{
    ...
    // notations
    PyModule_AddObject(module, "NOTATION_DEFAULT",
                       PyLong_FromLong(NOTATION_DEFAULT));
    ...
    PyModule_AddObject(module, "NOTATION_SHOGI_HOSKING",
                       PyLong_FromLong(NOTATION_SHOGI_HOSKING));
    PyModule_AddObject(module, "NOTATION_SHOGI_HODGES",
                       PyLong_FromLong(NOTATION_SHOGI_HODGES));
    PyModule_AddObject(module, "NOTATION_SHOGI_HODGES_NUMBER",
                       PyLong_FromLong(NOTATION_SHOGI_HODGES_NUMBER));
    PyModule_AddObject(module, "NOTATION_JANGGI",
                       PyLong_FromLong(NOTATION_JANGGI));
    PyModule_AddObject(module, "NOTATION_XIANGQI_WXF",
                       PyLong_FromLong(NOTATION_XIANGQI_WXF));
