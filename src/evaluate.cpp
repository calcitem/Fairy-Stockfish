...
[[maybe_unused]] const unsigned char *const gEmbeddedNNUEEnd =
    &gEmbeddedNNUEData[1];
...
void NNUE::init()
{
    ...
    // Support multiple variant networks separated by
    // semicolon(Windows)/colon(Unix)
    ...
    while (getline(ss, eval_file, UCI::SepChar)) {
        ...
        if (basename.rfind(variant, 0) != string::npos ||
            (!nnueAlias.empty() &&
             basename.rfind(nnueAlias, 0) != string::npos)) {
            useNNUE = true;
            break;
        }
    }
    ...
    for (string directory : dirs)
        if (eval_file_loaded != eval_file) {
            ...
            if (directory == "<internal>" && eval_file == EvalFileDefaultName) {
                ...
                MemoryBuffer buffer(
                    const_cast<char *>(
                        reinterpret_cast<const char *>(gEmbeddedNNUEData)),
...
void NNUE::verify()
{
    ...
    if (useNNUE && eval_file.find(eval_file_loaded) == string::npos) {
        ...
        string msg1 = "If the UCI option \"Use NNUE\" is set to true, network "
                      "evaluation parameters compatible with the engine must "
                      "be available.";
        ...
        string msg3 = "The UCI option EvalFile might need to specify the full "
                      "path, including the directory name, to the network "
                      "file.";
...
private:
    ...
    void initialize();
    template <Color Us>
    Score pieces(PieceType Pt);
    template <Color Us>
    Score hand(PieceType pt);
    template <Color Us>
    Score king() const;
    template <Color Us>
    Score threats() const;
    ...
    // attackedBy2[color] are the squares attacked by at least 2 units of a
    // given color, including x-rays. But diagonal x-rays through pawns are not
    // computed.
...
void Evaluation<T>::initialize()
{
    ...
    if (pos.must_capture())
        ...
        mobilityArea[Us] = ~(
            b | pos.pieces(Us, KING, QUEEN) | pos.blockers_for_king(Us) |
    ...
    attackedBy[Us][KING] = pos.count<KING>(Us) ?
                               pos.attacks_from(Us, KING, ksq) :
                               Bitboard(0);
    attackedBy[Us][PAWN] = pe->pawn_attacks(Us);
    attackedBy[Us][SHOGI_PAWN] = shift<Up>(pos.pieces(Us, SHOGI_PAWN));
    attackedBy[Us][ALL_PIECES] = attackedBy[Us][KING] | attackedBy[Us][PAWN] |
                                 attackedBy[Us][SHOGI_PAWN];
    ...
    if (!pos.count<KING>(Us))
        ...
        Square s = make_square(
            std::clamp(file_of(ksq), FILE_B, File(pos.max_file() - 1)),
    ...
    kingAttackersCount[Them] = popcount(
        kingRing[Us] &
        (pe->pawn_attacks(Them) | shift<Down>(pos.pieces(Them, SHOGI_PAWN))));
...
Score Evaluation<T>::pieces(PieceType Pt)
{
    ...
    constexpr Bitboard OutpostRanks = (Us == WHITE ?
    ...
    while (b1) {
        ...
        if (Pt > QUEEN)
            b = (b & pos.pieces()) |
                (pos.moves_from(Us, Pt, s) & ~pos.pieces() & pos.board_bb());
        ...
        if (pos.promoted_piece_type(Pt) != NO_PIECE_TYPE) {
            ...
            if (zone & (b | s))
                score += make_score(
                             PieceValue[MG][pos.promoted_piece_type(Pt)] -
                                 PieceValue[MG][Pt],
                             PieceValue[EG][pos.promoted_piece_type(Pt)] -
                                 PieceValue[EG][Pt]) /
                         (zone & s && b ? 6 : 12);
        } else if (pos.piece_demotion() && pos.unpromoted_piece_on(s))
            score -= make_score(
                         PieceValue[MG][Pt] -
                             PieceValue[MG][pos.unpromoted_piece_on(s)],
                         PieceValue[EG][Pt] -
                             PieceValue[EG][pos.unpromoted_piece_on(s)]) /
                     4;
        ...
        if ((pos.captures_to_hand() || pos.two_boards()) &&
            pos.count<KING>(Them) && pos.count<KING>(Us)) {
            ...
            score -= EndgameKingProximity *
                     (distance(s, pos.square<KING>(Us)) - 2);

        if (Pt == SOLDIER &&
            (pos.pieces(Us, SOLDIER) & rank_bb(s) & adjacent_files_bb(s)))
        ...
        if (Pt == BISHOP || Pt == KNIGHT) {
            ...
            if (pos.count<KING>(Us))
                score -= KingProtector[Pt == BISHOP] *
                         distance(pos.square<KING>(Us), s);

            if (Pt == BISHOP) {
                // Penalty according to the number of our pawns on the same
                // color square as the bishop, bigger when the center files are
                // blocked with pawns and smaller when the bishop is outside the
                // pawn chain.
                ...
                // Bonus for bishop on a long diagonal which can "see" both
                // center squares
                if (more_than_one(attacks_bb<BISHOP>(s, pos.pieces(PAWN)) &
                                  Center))
                ...
                if (pos.is_chess960() && (s == relative_square(Us, SQ_A1) ||
                    ...
                    Direction d = pawn_push(Us) +
                                  (file_of(s) == FILE_A ? EAST : WEST);
                    if (pos.piece_on(s + d) == make_piece(Us, PAWN))
                        score -= !pos.empty(s + d + pawn_push(Us)) ?
                                     4 * make_score(CorneredBishop,
                                                    CorneredBishop) :
                                     3 * make_score(CorneredBishop,
                                                    CorneredBishop);
                }
            }
        }
        ...
        if (Pt == ROOK) {
            ...
            if (pos.is_on_semiopen_file(Us, s)) {
                ...
                if (pos.pieces(Us, PAWN) & shift<Down>(pos.pieces()) &
                    file_bb(s)) {
    ...
    if (pos.count_in_hand(Us, pt) > 0 && pt != KING) {
        ...
        if ((b & kingRing[Them]) && pt != SHOGI_PAWN) {
            ...
            kingAttackersWeightInHand[Us] +=
                KingAttackWeights[std::min(pt, FAIRY_PIECES)] *
                pos.count_in_hand(Us, pt);
            kingAttacksCount[Us] += popcount(b & attackedBy[Them][KING]);
        }
        Bitboard theirHalf = pos.board_bb() &
                             ~forward_ranks_bb(
                                 Them, relative_rank(
                                           Them, Rank((pos.max_rank() - 1) / 2),
                                           pos.max_rank()));
        mobility[Us] += DropMobility *
                        popcount(b & theirHalf & ~attackedBy[Them][ALL_PIECES]);

        // Bonus for Kyoto shogi style drops of promoted pieces
        if (pos.promoted_piece_type(pt) != NO_PIECE_TYPE && pos.drop_promoted())
            score += make_score(
                         std::max(PieceValue[MG][pos.promoted_piece_type(pt)] -
                                      PieceValue[MG][pt],
                                  VALUE_ZERO),
                         std::max(PieceValue[EG][pos.promoted_piece_type(pt)] -
                                      PieceValue[EG][pt],
                                  VALUE_ZERO)) /
...
Score Evaluation<T>::king() const
{
    ...
    std::function<Bitboard(Color, PieceType)> get_attacks =
        [this](Color c, PieceType pt) {
            return attackedBy[c][pt] |
                   (pos.piece_drops() && pos.count_in_hand(c, pt) > 0 ?
    ...
    for (PieceType pt : pos.piece_types()) {
        switch (pt) {
        case QUEEN:
            // Enemy queen safe checks: we count them only if they are from
            // squares from which we can't give a rook check, because rook
            // checks are more valuable.
            queenChecks = (b1 | b2) & get_attacks(Them, QUEEN) &
                          pos.board_bb() & safe & ~attackedBy[Us][QUEEN] &
        ...
        case BISHOP:
        case KNIGHT:
            knightChecks = attacks_bb(Us, pt, ksq,
                                      pos.pieces() ^ pos.pieces(Us, QUEEN)) &
        ...
        case PAWN:
            if (pos.piece_drops() && pos.count_in_hand(Them, pt) > 0) {
                pawnChecks = attacks_bb(Us, pt, ksq, pos.pieces()) &
                             ~pos.pieces() & pos.board_bb();
                if (pawnChecks & safe)
                    kingDanger +=
                        SafeCheck[PAWN][more_than_one(pawnChecks & safe)];
                else
        ...
        case SHOGI_PAWN:
            if (pos.promoted_piece_type(pt)) {
                otherChecks = attacks_bb(Us, pos.promoted_piece_type(pt), ksq,
                                         pos.pieces()) &
                              attackedBy[Them][pt] &
                              zone_bb(Them, pos.promotion_rank(),
                                      pos.max_rank()) &
                              pos.board_bb();
                if (otherChecks & safe)
                    kingDanger += SafeCheck[FAIRY_PIECES]
                                           [more_than_one(otherChecks & safe)];
                else
        ...
        case KING:
            break;
        default:
            otherChecks = attacks_bb(Us, pt, ksq, pos.pieces()) &
                          get_attacks(Them, pt) & pos.board_bb();
            if (otherChecks & safe)
                kingDanger +=
                    SafeCheck[FAIRY_PIECES][more_than_one(otherChecks & safe)];
            else
                unsafeChecks |= otherChecks;
        }
    }
    ...
    if (pos.two_boards() && pos.piece_drops()) {
        for (PieceType pt : pos.piece_types())
            if (pos.count_in_hand(Them, pt) <= 0 &&
                (attacks_bb(Us, pt, ksq, pos.pieces()) & safe &
                 pos.drop_region(Them, pt) & ~pos.pieces())) {
    ...
    Square s = file_of(ksq) == FILE_A         ? ksq + EAST :
               file_of(ksq) == pos.max_file() ? ksq + WEST :
                                                ksq;
    Bitboard kingFlank = pos.max_file() == FILE_H ?
                             KingFlank[file_of(ksq)] :
                             file_bb(s) | adjacent_files_bb(s);
    ...
    kingDanger +=
        ...
        873 * !(pos.major_pieces(Them) || pos.captures_to_hand()) * 2 /
            (2 + 2 * pos.check_counting() + 2 * pos.two_boards() +
             2 * pos.makpong() +
    ...
    // evaluation
    if (kingDanger > 100)
        score -= make_score(std::min(kingDanger, 3500) * kingDanger / 4096,
                            kingDanger / 16);
    ...
    score -= FlankAttacks * kingFlankAttack *
             (1 + 5 * pos.captures_to_hand() + pos.check_counting());

    if (pos.check_counting())
        score += make_score(0, mg_value(score) * 2 /
                                   (2 + pos.checks_remaining(Them)));

    if (pos.king_type() == WAZIR)
        score += make_score(0, mg_value(score) / 2);

    // For drop games, king danger is independent of game phase, but dependent
    // on material density
    if (pos.captures_to_hand() || pos.two_boards())
...
Score Evaluation<T>::threats() const
{
    ...
    if (pos.must_capture()) {
        ...
        if (captures)
            score -= make_score(2000, 2000) /
                     (1 + popcount(captures & attackedBy[Them][ALL_PIECES] &
                                   ~attackedBy2[Us]));
        ...
        score += make_score(200, 200) *
                 popcount(attackedBy[Them][ALL_PIECES] & moves & ~pos.pieces());
        score += make_score(200, 220) * popcount(attackedBy[Them][ALL_PIECES] &
                                                 moves & ~pos.pieces() &
                                                 ~attackedBy2[Us]);
    }
    ...
    if (pos.extinction_value() == -VALUE_MATE) {
        ...
        for (PieceType pt : pos.extinction_piece_types()) {
            if (pt == ALL_PIECES)
                continue;
            int denom = std::max(pos.count_with_hand(Them, pt) -
                                     pos.extinction_piece_count(),
                                 1);
            // Explosion threats
            if (pos.blast_on_capture()) {
                int evasions = popcount(
                                   ((attackedBy[Them][pt] & ~pos.pieces(Them)) |
                                    pos.pieces(Them, pt)) &
                                   ~attackedBy[Us][ALL_PIECES]) *
                               denom;
                int attacks = popcount(
                    (attackedBy[Them][pt] | pos.pieces(Them, pt)) &
                    attackedBy[Us][ALL_PIECES]);
                int explosions = 0;

                Bitboard bExtBlast = bExt &
                                     (attackedBy2[Us] | ~attackedBy[Us][pt]);
                while (bExtBlast) {
    ...
    b = (pawn_attacks_bb<Us>(b) |
         shift<Up>(shift<Up>(pos.pieces(Us, SHOGI_PAWN, SOLDIER)))) &
        nonPawnEnemies;
...
Score Evaluation<T>::passed() const
{
    ...
    auto king_proximity = [&](Color c, Square s) {
        return pos.extinction_value() == VALUE_MATE ?
                   0 :
               pos.count<KING>(c) ?
                   std::min(distance(pos.square<KING>(c), s), 5) :
                   5;
    };
    ...
    while (b) {
        ...
        int r = std::max(RANK_8 -
                             std::max(pos.promotion_rank() -
                                          relative_rank(Us, s, pos.max_rank()),
                                      0),
                         0);
        ...
        if (r > RANK_3) {
            ...
            // If blockSq is not the queening square then consider also a second
            // push
            if (r != RANK_7)
            ...
            if (pos.empty(blockSq)) {
                ...
                // If there are no enemy pieces or attacks on passed pawn span,
                // assign a big bonus. Or if there is some, but they are all
                // attacked by our pawns, assign a bit smaller bonus. Otherwise
                // assign a smaller bonus if the path to queen is not attacked
                // and even smaller bonus if it is attacked but block square is
                // not.
    ...
    score = make_score(mg_value(score) * int(maxMg - PawnValueMg) /
                           (QueenValueMg - PawnValueMg),
                       eg_value(score) * int(maxEg - PawnValueEg) /
                           (QueenValueEg - PawnValueEg));
    ...
    if (pt != NO_PIECE_TYPE) {
        ...
        while (b) {
            ...
            Square blockSq = s + Up;
            int d = 2 * std::max(pos.promotion_rank() -
                                     relative_rank(Us, s, pos.max_rank()),
                                 1);
            d += !!(attackedBy[Them][ALL_PIECES] & ~attackedBy2[Us] & blockSq);
            score += make_score(PieceValue[MG][pt], PieceValue[EG][pt]) /
                     (d * d);
        }
    }
...
Score Evaluation<T>::space() const
{
    ...
    // exchanged
    if (pos.non_pawn_material() < SpaceThreshold && !pawnsOnly &&
        pos.double_step_enabled())
        return SCORE_ZERO;

    constexpr Color Them = ~Us;
    constexpr Direction Down = -pawn_push(Us);
    constexpr Bitboard SpaceMask = Us == WHITE ?
                                       CenterFiles &
    ...
    int weight = pos.count<ALL_PIECES>(Us) - 3 +
                 std::min(pe->blocked_count(), 9);
    Score score = make_score(bonus * weight * weight / 16, 0);

    if (pos.capture_the_flag(Us))
        score += make_score(200, 200) *
                 popcount(behind & safe & pos.capture_the_flag(Us));
...
// Evaluation::variant() computes variant-specific evaluation bonuses for a
// given side.
...
Score Evaluation<T>::variant() const
{
    ...
    if (pos.capture_the_flag(Us)) {
        ...
        Bitboard doubleBlocked = attackedBy2[Them] |
                                 ...
                                 (pawn_attacks_bb<Them>(
                                     pos.pieces(Them, PAWN) &
                                     pe->pawn_attacks(Them)));
        Bitboard inaccessible = pos.pieces(Us, PAWN) &
        ...
        // This reflects that likely a move will be needed to block or capture
        // the attack.
        for (int dist = 0;
             (ctfPieces || onHold || onHold2) && (ctfTargets & ~processed);
             dist++) {
            ...
            while (current) {
                ...
                Bitboard attacks = ((PseudoAttacks[Us][ptCtf][s] &
                                     pos.pieces()) |
                                    (PseudoMoves[Us][ptCtf][s] &
                                     ~pos.pieces())) &
    ...
    if (pos.extinction_value() != VALUE_NONE) {
        for (PieceType pt : pos.extinction_piece_types())
            if (pt != ALL_PIECES) {
                ...
                int denom = std::max(
                    pos.count(Us, pt) - pos.extinction_piece_count(), 1);
                if (pos.count(Them, pt) >=
                        pos.extinction_opponent_piece_count() ||
                    pos.two_boards())
                ...
                score += make_score(pos.non_pawn_material(Us),
                                    pos.non_pawn_material(Us)) /
                         std::max(pos.count<ALL_PIECES>(Us), 1);
            } else if (pos.count<PAWN>(Us) == pos.count<ALL_PIECES>(Us)) {
                // Pawns easy to stop/capture
                int l = 0, m = 0,
                    r = popcount(pos.pieces(Us, PAWN) & file_bb(FILE_A));
                for (File f = FILE_A; f <= pos.max_file(); ++f) {
                    l = m;
                    m = r;
                    r = popcount(pos.pieces(Us, PAWN) &
                                 shift<EAST>(file_bb(f)));
                    score -= make_score(
                                 80 - 10 * (edge_distance(f, pos.max_file()) %
                                            2),
                                 80 - 15 * (edge_distance(f, pos.max_file()) %
                                            2)) *
                             m / (1 + l * r);
                }
            } else if (pos.count<PAWN>(Them) == pos.count<ALL_PIECES>(Them)) {
                // Add a bonus according to how close we are to breaking through
                // the pawn wall
                int dist = 8;
                Bitboard breakthroughs = attackedBy[Us][ALL_PIECES] &
                                         rank_bb(relative_rank(Us,
                                                               pos.max_rank(),
                                                               pos.max_rank()));
                if (breakthroughs)
                    ...
                    for (File f = FILE_A; f <= pos.max_file(); ++f)
                        dist = std::min(dist, popcount(pos.pieces(PAWN) &
                                                       file_bb(f)));
                score += make_score(70, 70) * pos.count<PAWN>(Them) /
                         (1 + dist * dist) / (pos.pieces(Us, QUEEN) ? 2 : 4);
            }
    }
            ...
            while (b) {
                ...
                for (int j = 0; j < pos.connect_n(); j++)
                    if (pos.pieces(Us) & (s - j * d))
                        c++;
                score += make_score(200, 200) * c / (pos.connect_n() - c) /
                         (pos.connect_n() - c);
            }
        }
    }
    ...
    if (pos.flip_enclosed_pieces()) {
        ...
        if (pos.flip_enclosed_pieces() == REVERSI) {
            Bitboard edges = (FileABB | file_bb(pos.max_file()) | Rank1BB |
                              rank_bb(pos.max_rank())) &
                             pos.board_bb();
            Bitboard edgePieces = pos.pieces(Us) & edges;
            while (edgePieces) {
                Bitboard connectedEdge = attacks_bb(Us, ROOK,
                                                    pop_lsb(edgePieces),
                                                    ~(pos.pieces(Us) & edges)) &
                                         edges;
...
Value Evaluation<T>::winnable(Score score) const
{
    ...
    if (pos.extinction_value() == VALUE_NONE && !pos.captures_to_hand() &&
        !pos.connect_n() && !pos.material_counting()) {
        int outflanking = !pos.count<KING>(WHITE) || !pos.count<KING>(BLACK) ?
                              0 :
                              distance<File>(pos.square<KING>(WHITE),
                                             pos.square<KING>(BLACK)) +
                                  int(rank_of(pos.square<KING>(WHITE)) -
                                      rank_of(pos.square<KING>(BLACK)));
        ...
        bool infiltration = (pos.count<KING>(WHITE) &&
                             rank_of(pos.square<KING>(WHITE)) > RANK_4) ||
                            (pos.count<KING>(BLACK) &&
                             rank_of(pos.square<KING>(BLACK)) < RANK_5);

        // Compute the initiative bonus for the attacking side
        complexity = 9 * pe->passed_count() + 12 * pos.count<PAWN>() +
                     15 * pos.count<SOLDIER>() + 9 * outflanking +
                     21 * pawnsOnBothFlanks + 24 * infiltration +
                     51 * !pos.non_pawn_material() - 43 * almostUnwinnable -
                     110;
    }
    ...
    // bonus so that the midgame and endgame scores do not change sign after the
    // bonus.
    ...
    if (sf == SCALE_FACTOR_NORMAL && !pos.captures_to_hand() &&
        ...
        if (pos.opposite_bishops()) {
            ...
            // For every other opposite colored bishops endgames use scale
            // factor based on the number of all pieces of the strong side.
            else
                sf = 22 + 3 * pos.count<ALL_PIECES>(strongSide);
        }
        // For rook endgames with strong side not having overwhelming pawn
        // number advantage and its pawns being on one flank and weak side
        // protecting its pieces with a king use lower scale factor.
        ...
        // based on number of minors of side that doesn't have queen.
            ...
            sf = 37 +
                 3 * (pos.count<QUEEN>(WHITE) == 1 ?
                          pos.count<BISHOP>(BLACK) + pos.count<KNIGHT>(BLACK) :
                          pos.count<BISHOP>(WHITE) + pos.count<KNIGHT>(WHITE));
        // In every other case use scale factor based on
        // the number of pawns of the strong side reduced if pawns are on a
        // single flank.
        else
    ...
    v = mg * int(me->game_phase()) + eg *
                                         int(PHASE_MIDGAME - me->game_phase()) *
...
Value Evaluation<T>::value()
{
    ...
    score += (mobility[WHITE] - mobility[BLACK]) *
             (1 + pos.captures_to_hand() + pos.must_capture() +
              pos.check_counting());
    ...
    score += threats<WHITE>() - threats<BLACK>() + space<WHITE>() -
             space<BLACK>();
...
{
    constexpr Bitboard Corners = Bitboard(1ULL) << SQ_A1 |
                                 Bitboard(1ULL) << SQ_H1 |
                                 Bitboard(1ULL) << SQ_A8 |
                                 Bitboard(1ULL) << SQ_H8;
...
Value Eval::evaluate(const Position &pos)
{
    ...
    if (!Eval::useNNUE || !pos.nnue_applicable())
        ...
        // evaluation
        auto adjusted_NNUE = [&]() {
            int scale = 903 + 32 * pos.count<PAWN>() +
        ...
        // NNUE eval faster when shuffling or if the material on the board is
        // high.
        int r50 = pos.rule50_count();
        Value psq = Value(abs(eg_value(pos.psq_score())));
        bool pure = !pos.check_counting();
        bool classical = psq * 5 >
                             (750 + pos.non_pawn_material() / 64) * (5 + r50) &&
                         !pure;
    ...
    v = std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1,
...
std::string Eval::trace(Position &pos)
{
    ...
    std::stringstream ss;
    ss << std::showpoint << std::noshowpos << std::fixed
       << std::setprecision(2);
    ...
    ss << std::showpoint << std::noshowpos << std::fixed << std::setprecision(2)
       ...
       << Term(IMBALANCE) << "|      Pawns | " << Term(PAWN)
       << "|    Knights | " << Term(KNIGHT) << "|    Bishops | " << Term(BISHOP)
       << "|      Rooks | " << Term(ROOK) << "|     Queens | " << Term(QUEEN)
       << "|   Mobility | " << Term(MOBILITY) << "|King safety | " << Term(KING)
       << "|    Threats | " << Term(THREAT) << "|     Passed | " << Term(PASSED)
       << "|      Space | " << Term(SPACE) << "|    Variant | " << Term(VARIANT)
