...
/// Variant struct stores information needed to determine the rules of a
/// variant.
...
struct Variant
{
    ...
    std::string pieceToChar = " PNBRQ" + std::string(KING - QUEEN - 1, ' ') +
    ...
    std::set<PieceType, std::greater<PieceType>> promotionPieceTypes = {
    ...
    bool stalematePieceCount = false; // multiply stalemate value by
                                      // sign(count(~stm) - count(stm))
    ...
    void add_piece(PieceType pt, char c, char c2) { add_piece(pt, c, "", c2); }
    ...
    Variant *conclude()
    {
        fastAttacks = std::all_of(
                          pieceTypes.begin(), pieceTypes.end(),
                          [this](PieceType pt) {
                              return (pt < FAIRY_PIECES || pt == COMMONER ||
                                      pt == IMMOBILE_PIECE ||
                                      pt == ARCHBISHOP || pt == CHANCELLOR ||
                                      (pt == KING && kingType == KING)) &&
                                     !(mobilityRegion[WHITE][pt] ||
                                       mobilityRegion[BLACK][pt]);
                          }) &&
                      !cambodianMoves && !diagonalLines;
        fastAttacks2 = std::all_of(
                           pieceTypes.begin(), pieceTypes.end(),
                           [this](PieceType pt) {
                               return (pt < FAIRY_PIECES || pt == COMMONER ||
                                       pt == FERS || pt == WAZIR ||
                                       pt == BREAKTHROUGH_PIECE ||
                                       pt == SHOGI_PAWN || pt == GOLD ||
                                       pt == SILVER || pt == SHOGI_KNIGHT ||
        ...
        nnueKing = pieceTypes.find(KING) != pieceTypes.end() ? KING :
                   extinctionPieceCount == 0 &&
                           extinctionPieceTypes.find(COMMONER) !=
                               extinctionPieceTypes.end() ?
                                                               COMMONER :
                                                               NO_PIECE_TYPE;
        ...
        nnueUsePockets = (pieceDrops &&
                          (capturesToHand || (!mustDrop && !arrowGating &&
                                              pieceTypes.size() != 1))) ||
                         seirawanGating;
        int nnuePockets = nnueUsePockets ? 2 * int(maxFile + 1) : 0;
        int nnueNonDropPieceIndices = (2 * pieceTypes.size() -
                                       (nnueKing != NO_PIECE_TYPE)) *
                                      nnueSquares;
        int nnuePieceIndices = nnueNonDropPieceIndices +
                               2 *
                                   (pieceTypes.size() -
                                    (nnueKing != NO_PIECE_TYPE)) *
                                   nnuePockets;
        ...
        for (PieceType pt : pieceTypes) {
            for (Color c : {WHITE, BLACK}) {
                ...
                pieceSquareIndex[c][make_piece(~c, pt)] = (2 * i +
                                                           (pt != nnueKing)) *
                                                          nnueSquares;
                pieceHandIndex[c][make_piece(c, pt)] = 2 * i * nnuePockets +
                                                       nnueNonDropPieceIndices;
                pieceHandIndex[c][make_piece(~c, pt)] = (2 * i + 1) *
                                                            nnuePockets +
                                                        nnueNonDropPieceIndices;
            }
            i++;
        ...
        // Variants might be initialized before bitboards, so do not rely on
        // precomputed bitboards (like SquareBB).
        int nnueKingSquare = 0;
        if (nnueKing)
            for (Square s = SQ_A1; s < nnueSquares; ++s) {
                Square bitboardSquare = Square(s + s / (maxFile + 1) *
                                                       (FILE_MAX - maxFile));
                if (!mobilityRegion[WHITE][nnueKing] ||
                    !mobilityRegion[BLACK][nnueKing] ||
                    (mobilityRegion[WHITE][nnueKing] &
                     make_bitboard(bitboardSquare)) ||
                    (mobilityRegion[BLACK][nnueKing] &
                     make_bitboard(
                         relative_square(BLACK, bitboardSquare, maxRank)))) {
        ...
        // For endgame evaluation to be applicable, no special win rules must
        // apply. Furthermore, rules significantly changing game mechanics also
        // invalidate it.
        endgameEval = std::none_of(pieceTypes.begin(), pieceTypes.end(),
                                   [this](PieceType pt) {
                                       return mobilityRegion[WHITE][pt] ||
                                              mobilityRegion[BLACK][pt];
                                   }) &&
                      extinctionValue == VALUE_NONE &&
                      checkmateValue == -VALUE_MATE &&
                      stalemateValue == VALUE_DRAW && !materialCounting &&
                      !flagPiece && !mustCapture && !checkCounting &&
                      !makpongRule && !connectN && !blastOnCapture &&
                      !capturesToHand && !twoBoards && kingType == KING;
