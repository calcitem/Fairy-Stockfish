...
#endif
#ifdef LARGEBOARDS
constexpr Bitboard DarkSquares = (Bitboard(0xAAA555AAA555AAULL) << 64) ^
                                 Bitboard(0xA555AAA555AAA555ULL);
#else
constexpr Bitboard DarkSquares = 0xAA55AA55AA55AA55ULL;
#endif

#ifdef LARGEBOARDS
constexpr Bitboard FileABB = (Bitboard(0x00100100100100ULL) << 64) ^
                             Bitboard(0x1001001001001001ULL);
#else
...
{
    return (from_sq(m) != SQ_NONE && (b & to_sq(m))) ?
               (b ^ to_sq(m)) | from_sq(m) :
               b;
}
...
inline Bitboard between_bb(Square s1, Square s2, PieceType pt)
{
    if (pt == HORSE)
        ...
        return (PseudoAttacks[WHITE][WAZIR][s2] &
                PseudoAttacks[WHITE][ALFIL][s1]) |
               (PseudoAttacks[WHITE][KNIGHT][s2] &
                PseudoAttacks[WHITE][FERS][s1]);
    else
...
{
    return c == WHITE ? (AllSquares ^ Rank1BB)
                            << FILE_NB * relative_rank(WHITE, s, RANK_MAX) :
                        (AllSquares ^ rank_bb(RANK_MAX)) >>
                            FILE_NB * relative_rank(BLACK, s, RANK_MAX);
}

constexpr Bitboard forward_ranks_bb(Color c, Rank r)
{
    return c == WHITE ?
               (AllSquares ^ Rank1BB) << FILE_NB * (r - RANK_1) :
               (AllSquares ^ rank_bb(RANK_MAX)) >> FILE_NB * (RANK_MAX - r);
}

/// zone_bb() returns a bitboard representing the squares on all the ranks
/// in front of and on the given relative rank, from the point of view of the
/// given color. For instance, zone_bb(BLACK, RANK_7) will return the 16 squares
/// on ranks 1 and 2.
...
inline int popcount(Bitboard b)
{
    ...
    return PopCnt16[v.u[0]] + PopCnt16[v.u[1]] + PopCnt16[v.u[2]] +
           PopCnt16[v.u[3]] + PopCnt16[v.u[4]] + PopCnt16[v.u[5]] +
           PopCnt16[v.u[6]] + PopCnt16[v.u[7]];
#else
    union {
        Bitboard bb;
        uint16_t u[4];
...
#ifdef LARGEBOARDS
    return (int)_mm_popcnt_u64(uint64_t(b >> 64)) +
           (int)_mm_popcnt_u64(uint64_t(b));
#else
