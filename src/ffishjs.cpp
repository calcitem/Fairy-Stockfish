...
{
    if (uciVariant.size() == 0 || uciVariant == "Standard" ||
        uciVariant == "standard")
...
template <bool isUCI>
inline bool is_move_none(Move move, const std::string &strMove,
                         const Position &pos)
{
    if (move == MOVE_NONE) {
        std::cerr << "The given ";
        isUCI ? std::cerr << "uciMove" : std::cerr << "sanMove";
        std::cerr << " '" << strMove << "' for position '" << pos.fen()
                  << "' is invalid." << std::endl;
        return true;
    }
    return false;
...
class Board
{
    // note: we can't use references for strings here due to conversion to
    // JavaScript
private:
    const Variant *v;
    StateListPtr states;
...
public:
    ...
    // TODO: This is a naive implementation which compares all legal SAN moves
    // with the requested string. If the SAN move wasn't found the position
    // remains unchanged. Alternatively, implement a direct conversion.
    ...
    std::string fen() const { return this->pos.fen(); }
    ...
    std::string san_move(std::string uciMove, Notation notation)
    {
        ...
        if (is_move_none<true>(move, uciMove, pos))
            return "";
        return SAN::move_to_san(this->pos, UCI::to_move(this->pos, uciMove),
                                notation);
    }
                              ...
                              bool moveNumbers)
    {
        ...
        while (std::getline(ss, uciMove, ' ')) {
            ...
            if (first) {
                ...
                variationSan += SAN::move_to_san(this->pos, moves.back(),
                                                 Notation(notation));
            } else {
    ...
    bool is_game_over() const { return is_game_over(false); }
    ...
    std::string result() const { return result(false); }
    ...
    std::string result(bool claim_draw) const
    {
        ...
        if (!gameEnd && MoveList<LEGAL>(pos).size() == 0) {
            gameEnd = true;
            result = pos.checkers() ? pos.checkmate_value() :
                                      pos.stalemate_value();
        }
    ...
    bool is_check() const { return Stockfish::is_check(pos); }
    ...
    std::string pocket(bool color)
    {
        ...
        for (PieceType pt = KING; pt >= PAWN; --pt) {
            for (int i = 0; i < pos.count_in_hand(c, pt); ++i) {
                ...
                pocket += std::string(
                    1, pos.piece_to_char()[make_piece(BLACK, pt)]);
            }
        }
        return pocket;
    }
...
bool skip_comment(const std::string &pgn, size_t &curIdx, size_t &lineEnd)
{
    ...
    if (curIdx == std::string::npos) {
        std::cerr << "Missing '}' for move comment while reading pgn."
                  << std::endl;
        return false;
    }
    if (curIdx > lineEnd)
...
Game read_game_pgn(std::string pgn)
{
    ...
    while (true) {
        ...
        if (!headersParsed && pgn[lineStart] == '[') {
            ...
            // put item into list
            game.header[pgn.substr(headerKeyStart,
                                   headerKeyEnd - headerKeyStart)] =
            ...
            if (!headersParsed) {
                ...
                if (it != game.header.end()) {
                    game.is960 = it->second.find("960",
                                                 it->second.size() - 3) !=
                                 std::string::npos;
                    if (game.is960) {
                        game.variant = it->second.substr(0,
                                                         it->second.size() - 3);
                    } else {
                        game.variant = it->second;
                    }
                    std::transform(game.variant.begin(), game.variant.end(),
                                   game.variant.begin(), [](unsigned char c) {
                ...
                game.board = std::make_unique<Board>(game.variant, game.fen,
                                                     game.is960);
            ...
            while (curIdx <= lineEnd) {
                ...
                if (sanMoveEnd > curIdx) {
                    std::string sanMove = pgn.substr(curIdx,
                                                     sanMoveEnd - curIdx);
                    ...
                    if (annotationChar1 != std::string::npos ||
                        ...
                        sanMove = sanMove.substr(0, std::min(annotationChar1,
                                                             annotationChar2));
                    std::cout << sanMove << " ";
...
EMSCRIPTEN_BINDINGS(ffish_js)
{
    class_<Board>("Board")
        ...
        .function("pushSan",
                  select_overload<bool(std::string)>(&Board::push_san))
        .function("pushSan", select_overload<bool(std::string, Notation)>(
                                 &Board::push_san))
        ...
        .function("fen",
                  select_overload<std::string(bool, int) const>(&Board::fen))
        .function("setFen", &Board::set_fen)
        .function("sanMove",
                  select_overload<std::string(std::string)>(&Board::san_move))
        .function("sanMove",
                  select_overload<std::string(std::string, Notation)>(
                      &Board::san_move))
        .function("variationSan", select_overload<std::string(std::string)>(
                                      &Board::variation_san))
        .function("variationSan",
                  select_overload<std::string(std::string, Notation)>(
                      &Board::variation_san))
        .function("variationSan",
                  select_overload<std::string(std::string, Notation, bool)>(
                      &Board::variation_san))
        ...
        .function("isGameOver",
                  select_overload<bool() const>(&Board::is_game_over))
        .function("isGameOver",
                  select_overload<bool(bool) const>(&Board::is_game_over))
        .function("result",
                  select_overload<std::string() const>(&Board::result))
        .function("result",
                  select_overload<std::string(bool) const>(&Board::result))
        ...
        .function("pushSanMoves",
                  select_overload<void(std::string)>(&Board::push_san_moves))
        .function("pushSanMoves", select_overload<void(std::string, Notation)>(
                                      &Board::push_san_moves))
    ...
    function("validateFen",
             select_overload<int(std::string)>(&ffish::validate_fen));
    function("validateFen", select_overload<int(std::string, std::string)>(
                                &ffish::validate_fen));
    function("validateFen",
             select_overload<int(std::string, std::string, bool)>(
                 &ffish::validate_fen));
    // TODO: enable to string conversion method
    // .class_function("getStringFromInstance",
    // &Board::get_string_from_instance);
}
