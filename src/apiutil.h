...
inline std::string piece(const Position &pos, Move m, Notation n)
{
    ...
    // Tandem pawns
             ...
             popcount(pos.pieces(us, pt) & file_bb(from)) >=
                 3 - multi_tandem(pos.pieces(us, pt)))
        return std::to_string(
            popcount(forward_file_bb(us, from) & pos.pieces(us, pt)) + 1);
    // Moves of promoted pieces
    else if (is_shogi(n) && type_of(m) != DROP && pos.unpromoted_piece_on(from))
        return "+" +
               std::string(
                   1,
                   toupper(pos.piece_to_char()[pos.unpromoted_piece_on(from)]));
    // Promoted drops
    else if (is_shogi(n) && type_of(m) == DROP &&
             dropped_piece_type(m) != in_hand_piece_type(m))
        return "+" +
               std::string(1,
                           toupper(pos.piece_to_char()[in_hand_piece_type(m)]));
...
inline std::string file(const Position &pos, Square s, Notation n)
{
    ...
    case NOTATION_XIANGQI_WXF:
        return std::to_string((pos.side_to_move() == WHITE ?
                                   pos.max_file() - file_of(s) :
                                   file_of(s)) +
                              1);
    default:
...
inline std::string rank(const Position &pos, Square s, Notation n)
{
    ...
    case NOTATION_XIANGQI_WXF: {
        if (pos.empty(s))
            // Handle piece drops
            return std::to_string(
                relative_rank(pos.side_to_move(), s, pos.max_rank()) + 1);
...
inline Disambiguation disambiguation_level(const Position &pos, Move m,
                                           Notation n)
{
...
inline std::string disambiguation(const Position &pos, Square s, Notation n,
                                  Disambiguation d)
{
    switch (d) {
...
inline const std::string move_to_san(Position &pos, Move m, Notation n)
{
    ...
    if (type_of(m) == CASTLING) {
        ...
        if (is_gating(m)) {
            san += std::string("/") +
                   (char)toupper(
                       pos.piece_to_char()[make_piece(us, gating_type(m))]);
        ...
        if (type_of(m) == PROMOTION)
            san += std::string("=") +
                   (char)toupper(
                       pos.piece_to_char()[make_piece(us, promotion_type(m))]);
        else if (type_of(m) == PIECE_PROMOTION)
            san += is_shogi(n) ?
                       std::string("+") :
                       std::string("=") +
                           (char)toupper(pos.piece_to_char()[make_piece(
                               us, pos.promoted_piece_type(
...
inline bool has_insufficient_material(Color c, const Position &pos)
{
    ...
    if (pos.captures_to_hand() || pos.count_in_hand(c, ALL_PIECES) ||
        ...
        (pos.capture_the_flag_piece() &&
         pos.count(c, pos.capture_the_flag_piece())))
        return false;
    ...
    if (pos.extinction_pseudo_royal() && pos.blast_on_capture() &&
        pos.extinction_piece_types().find(COMMONER) !=
    ...
    // Mating pieces
    for (PieceType pt :
         {ROOK, QUEEN, ARCHBISHOP, CHANCELLOR, SILVER, GOLD, COMMONER, CENTAUR})
        if ((pos.pieces(c, pt) & ~restricted) ||
            (pos.count(c, PAWN) && pos.promotion_piece_types().find(pt) !=
                                       pos.promotion_piece_types().end()))
            return false;
    ...
    if ((colorbound & pos.pieces(c)) &&
        (((DarkSquares & colorbound) && (~DarkSquares & colorbound)) ||
         unbound || pos.stalemate_value() != VALUE_DRAW ||
         pos.check_counting() || pos.makpong()))
        return false;

    // Unbound pieces require one helper piece of either color
    if ((pos.pieces(c) & unbound) &&
        (popcount(pos.pieces() ^ restricted) >= 2 ||
         pos.stalemate_value() != VALUE_DRAW || pos.check_counting() ||
         pos.makpong()))
        return false;
...
inline bool is_check(const Position &pos)
{
    return pos.checkers() ||
           (pos.extinction_pseudo_royal() &&
            pos.attackers_to_pseudo_royals(~pos.side_to_move()));
}
...
inline int non_root_euclidian_distance(const CharSquare &s1,
                                       const CharSquare &s2)
{
...
private:
    int nbRanks;
    int nbFiles;
    std::vector<char> board; // fill an array where the pieces are for later
                             // geometry checks
public:
    CharBoard(int ranks, int files)
        : nbRanks(ranks)
        , nbFiles(files)
    {
    ...
    int get_nb_ranks() const { return nbRanks; }
    int get_nb_files() const { return nbFiles; }
...
inline Validation
check_for_valid_characters(const std::string &firstFenPart,
                           const std::string &validSpecialCharactersFirstField,
                           const Variant *v)
{
    for (char c : firstFenPart) {
        if (!isdigit(c) && !in_any({v->pieceToChar, v->pieceToCharSynonyms,
...
inline std::vector<std::string> get_fen_parts(const std::string &fullFen,
                                              char delim)
{
...
inline Validation
fill_char_board(CharBoard &board, const std::string &fenBoard,
                const std::string &validSpecialCharactersFirstField,
                const Variant *v)
{
    int rankIdx = 0;
    ...
    for (char c : fenBoard) {
        ...
        if (isdigit(c)) {
            fileIdx += c - '0';
            // if we have multiple digits attached we can add multiples of 9 to
            // compute the resulting number (e.g. -> 21 = 2 + 2 * 9 + 1)
            if (isdigit(prevChar))
            ...
            if (rankIdx == board.get_nb_ranks())
                break;
            fileIdx = 0;
        } else if (!contains(validSpecialCharactersFirstField, c)) { // normal
                                                                     // piece
            if (fileIdx == board.get_nb_files()) {
            ...
            board.set_piece(v->maxRank - rankIdx, fileIdx, c); // we mirror the
                                                               // rank index
                                                               // because the
                                                               // black pieces
                                                               // are given
    ...
    if (v->pieceDrops) { // pockets can either be defined by [] or /
        if (rankIdx + 1 != board.get_nb_ranks() &&
            rankIdx != board.get_nb_ranks()) {
            std::cerr << "Invalid number of ranks. Expected: "
                      << board.get_nb_ranks() << " Actual: " << rankIdx + 1
                      << std::endl;
            return NOK;
        }
    } else {
...
inline Validation
check_touching_kings(const CharBoard &board,
                     const std::array<CharSquare, 2> &kingPositions)
{
    if (non_root_euclidian_distance(kingPositions[WHITE],
                                    kingPositions[BLACK]) <= 2) {
                            ...
                            std::array<std::string, 2> &castlingInfoSplitted)
{
    for (char c : castlingInfo) {
        if (c != '-') {
            if (!isalpha(c)) {
                std::cerr << "Invalid castling specification: '" << c << "'."
                          << std::endl;
                return NOK;
            } else if (isupper(c))
...
inline Validation
check_castling_rank(const std::array<std::string, 2> &castlingInfoSplitted,
            ...
            if (tolower(castlingFlag) == 'k' || tolower(castlingFlag) == 'q') {
                if (kingPositions[c].rowIdx != castlingRank) {
                    std::cerr << "The " << color_to_string(c)
                              << " king must be on rank " << castlingRank
                              << " if castling is enabled for "
                ...
                if (!castlingRook) {
                    std::cerr << "No castling rook for flag " << castlingFlag
                              << std::endl;
                    return NOK;
                }
            } else if (board.get_piece(castlingRank,
                                       tolower(castlingFlag) - 'a') == ' ') {
                std::cerr << "No gating piece for flag " << castlingFlag
                          << std::endl;
                return NOK;
            }
        }
...
inline Validation check_standard_castling(
    std::array<std::string, 2> &castlingInfoSplitted, const CharBoard &board,
    const std::array<CharSquare, 2> &kingPositions,
    ...
    const Variant *v)
{
    for (Color c : {WHITE, BLACK}) {
        ...
        if (kingPositions[c] != kingPositionsStart[c]) {
            std::cerr << "The " << color_to_string(c)
                      << " KING has moved. Castling is no longer valid for "
                      << color_to_string(c) << "." << std::endl;
            return NOK;
        }

        for (CastlingRights castling : {KING_SIDE, QUEEN_SIDE}) {
            CharSquare rookStartingSquare = castling == QUEEN_SIDE ?
                                                rookPositionsStart[c][0] :
                                                rookPositionsStart[c][1];
            char targetChar = castling == QUEEN_SIDE ? 'q' : 'k';
            char rookChar = v->pieceToChar[make_piece(c, v->castlingRookPiece)];
            if (castlingInfoSplitted[c].find(targetChar) != std::string::npos) {
                if (board.get_piece(rookStartingSquare.rowIdx,
                                    rookStartingSquare.fileIdx) != rookChar) {
                    std::cerr << "The " << color_to_string(c) << " ROOK on the "
                              << castling_rights_to_string(castling)
                              << " has moved. "
                              << castling_rights_to_string(castling)
                              << " castling is no longer valid for "
                              << color_to_string(c) << "." << std::endl;
                    return NOK;
                }
            }
...
inline Validation check_pocket_info(const std::string &fenBoard, int nbRanks,
                                    const Variant *v, std::string &pocket)
{
    char stopChar;
    int offset = 0;
    if (std::count(fenBoard.begin(), fenBoard.end(), '/') == nbRanks) {
        ...
        offset = 1;
        if (*(fenBoard.end() - 1) != ']') {
            std::cerr << "Pocket specification does not end with ']'."
                      << std::endl;
            return NOK;
        }
    } else
    ...
    for (auto it = fenBoard.rbegin() + offset; it != fenBoard.rend(); ++it) {
        ...
        if (c != '-') {
            if (!in_any({v->pieceToChar, v->pieceToCharSynonyms}, c)) {
                std::cerr << "Invalid pocket piece: '" << c << "'."
                          << std::endl;
                return NOK;
            } else
                pocket += c;
    ...
    std::cerr << "Pocket piece closing character '" << stopChar
              << "' was not found." << std::endl;
    return NOK;
}

inline int piece_count(const std::string &fenBoard, Color c, PieceType pt,
                       const Variant *v)
{
    return std::count(fenBoard.begin(), fenBoard.end(),
                      v->pieceToChar[make_piece(c, pt)]);
}

inline Validation check_number_of_kings(const std::string &fenBoard,
                                        const std::string &startFenBoard,
                                        const Variant *v)
{
    ...
    if (nbWhiteKings != nbWhiteKingsStart) {
        std::cerr << "Invalid number of white kings. Expected: "
                  << nbWhiteKingsStart << ". Given: " << nbWhiteKings
                  << std::endl;
        return NOK;
    }
    if (nbBlackKings != nbBlackKingsStart) {
        std::cerr << "Invalid number of black kings. Expected: "
                  << nbBlackKingsStart << ". Given: " << nbBlackKings
                  << std::endl;
        return NOK;
    }
    return OK;
    ...
    if (enPassantInfo.size() != 1 || enPassantInfo[0] != '-') {
        if (enPassantInfo.size() != 2) {
            std::cerr << "Invalid en-passant square '" << enPassantInfo
                      << "'. Expects 2 characters. Actual: "
                      << enPassantInfo.size() << " character(s)." << std::endl;
            return NOK;
        }
        if (!isalpha(enPassantInfo[0])) {
            std::cerr << "Invalid en-passant square '" << enPassantInfo
                      << "'. Expects 1st character to be a letter."
                      << std::endl;
            return NOK;
        }
...
inline Validation check_check_count(const std::string &checkCountInfo)
{
    if (checkCountInfo.size() != 3) {
        std::cerr << "Invalid check count '" << checkCountInfo
                  << "'. Expects 3 characters. Actual: "
                  << checkCountInfo.size() << " character(s)." << std::endl;
        return NOK;
    }
...
inline Validation check_lichess_check_count(const std::string &checkCountInfo)
{
    if (checkCountInfo.size() != 4) {
        std::cerr << "Invalid check count '" << checkCountInfo
                  << "'. Expects 4 characters. Actual: "
                  << checkCountInfo.size() << " character(s)." << std::endl;
        return NOK;
    }
    if (!isdigit(checkCountInfo[1]) || checkCountInfo[1] - '0' > 3) {
        std::cerr << "Invalid check count '" << checkCountInfo
                  << "'. Expects 2nd character to be a digit up to 3."
                  << std::endl;
        return NOK;
    }
    if (!isdigit(checkCountInfo[3]) || checkCountInfo[3] - '0' > 3) {
        std::cerr << "Invalid check count '" << checkCountInfo
                  << "'. Expects 4th character to be a digit up to 3."
                  << std::endl;
        return NOK;
    }
    return OK;
...
inline std::string get_valid_special_chars(const Variant *v)
{
    ...
    // Whether or not '-', '+', '~', '[', ']' are valid depends on the variant
    // being played.
...
inline FenValidation validate_fen(const std::string &fen, const Variant *v,
                                  bool chess960 = false)
{
    const std::string validSpecialCharactersFirstField = get_valid_special_chars(
        v);
    // 0) Layout
    // check for empty fen
    if (fen.empty()) {
    ...
    if (fenParts.size() < 1 || fenParts.size() > maxNumberFenParts) {
        std::cerr << "Invalid number of fen parts. Expected: >= 1 and <= "
                  << maxNumberFenParts << " Actual: " << fenParts.size()
                  << std::endl;
    ...
    if (check_for_valid_characters(fenParts[0],
                                   validSpecialCharactersFirstField, v) == NOK)
    ...
    CharBoard board(nbRanks, nbFiles); // create a 2D character board for later
                                       // geometry checks

    if (fill_char_board(board, fenParts[0], validSpecialCharactersFirstField,
                        v) == NOK)
    ...
    if (v->pieceTypes.find(KING) != v->pieceTypes.end()) {
        ...
        // ensure that each side has exactly as many kings as in the starting
        // position (variants like giveaway use the COMMONER piece type instead)
        if (check_number_of_kings(fenParts[0], startFenParts[0], v) == NOK)
            return FEN_INVALID_NUMBER_OF_KINGS;

        // check for touching kings if there are exactly two royal kings on the
        // board (excluding pocket)
        if (v->kingType == KING &&
            piece_count(fenParts[0], WHITE, KING, v) -
                    piece_count(pocket, WHITE, KING, v) ==
                1 &&
            piece_count(fenParts[0], BLACK, KING, v) -
                    piece_count(pocket, BLACK, KING, v) ==
                1) {
            std::array<CharSquare, 2> kingPositions;
            kingPositions[WHITE] = board.get_square_for_piece(
                v->pieceToChar[make_piece(WHITE, KING)]);
            kingPositions[BLACK] = board.get_square_for_piece(
                v->pieceToChar[make_piece(BLACK, KING)]);
    ...
    if (fenParts.size() >= 2 && fenParts[1][0] != 'w' &&
        fenParts[1][0] != 'b') {
    ...
    bool skipCastlingAndEp = fenParts.size() >= 4 && fenParts.size() <= 5 &&
                             isdigit(fenParts[2][0]);

    // 3) Part
    // check castling rights
    if (fenParts.size() >= 3 && !skipCastlingAndEp && v->castling) {
        std::array<std::string, 2> castlingInfoSplitted;
        if (fill_castling_info_splitted(fenParts[2], castlingInfoSplitted) ==
            NOK)
            return FEN_INVALID_CASTLING_INFO;

        if (castlingInfoSplitted[WHITE].size() != 0 ||
            castlingInfoSplitted[BLACK].size() != 0) {
            std::array<CharSquare, 2> kingPositions;
            kingPositions[WHITE] = board.get_square_for_piece(
                toupper(v->pieceToChar[v->castlingKingPiece]));
            kingPositions[BLACK] = board.get_square_for_piece(
                tolower(v->pieceToChar[v->castlingKingPiece]));

            CharBoard startBoard(board.get_nb_ranks(), board.get_nb_files());
            fill_char_board(startBoard, v->startFen,
                            validSpecialCharactersFirstField, v);

            // Check pieces present on castling rank against castling/gating
            // rights
            if (check_castling_rank(castlingInfoSplitted, board, kingPositions,
                                    v) == NOK)
                return FEN_INVALID_CASTLING_INFO;

            // only check exact squares if starting position of castling pieces
            // is known
            if (!v->chess960 && !v->castlingDroppedPiece && !chess960) {
                ...
                if (check_standard_castling(castlingInfoSplitted, board,
                                            kingPositions, kingPositionsStart,
                                            rookPositionsStart, v) == NOK)
                    return FEN_INVALID_CASTLING_INFO;
            }
        }
    }
    ...
    if (fenParts.size() >= 3 + optionalInbetweenFields &&
        !check_digit_field(
            fenParts[fenParts.size() - 2 - optionalTrailingFields])) {
        std::cerr << "Invalid half move counter: '"
                  << fenParts[fenParts.size() - 2] << "'." << std::endl;
    ...
    if (fenParts.size() >= 4 + optionalInbetweenFields &&
        !check_digit_field(
            fenParts[fenParts.size() - 1 - optionalTrailingFields])) {
