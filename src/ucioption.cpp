...
std::set<string> standard_variants = {
    "normal",     "nocastle", "fischerandom", "knightmate", "3check",
    "makruk",     "shatranj", "asean",        "seirawan",   "crazyhouse",
    "bughouse",   "suicide",  "giveaway",     "losers",     "atomic",
    "capablanca", "gothic",   "janus",        "caparandom", "grand",
    "shogi",      "xiangqi"};
...
void on_variant_change(const Option &o)
{
    ...
    if (standard_variants.find(o) != standard_variants.end())
        return;
    int pocketsize = v->pieceDrops ? (v->pocketSize ? v->pocketSize :
                                                      v->pieceTypes.size()) :
                                     0;
    if (CurrentProtocol == XBOARD) {
        ...
        if (itJanggi != variants.end() &&
            ...
            sync_cout << "setup (PH.R.AE..K.C.ph.r.ae..k.c.) 9x10+0_janggi "
                      << "rhea1aehr/4k4/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/"
                         "4K4/RHEA1AEHR w - - 0 1"
                      << sync_endl;
            return;
        }
        // Send setup command
        sync_cout << "setup (" << v->pieceToCharTable << ") " << v->maxFile + 1
                  << "x" << v->maxRank + 1 << "+" << pocketsize << "_"
                  << v->variantTemplate << " " << v->startFen << sync_endl;
        ...
        for (PieceType pt : v->pieceTypes) {
            ...
            if (pt == KING && v->castling)
                suffix += "O" + std::to_string((v->castlingKingsideFile -
            ...
            if (v->pieceDrops) {
                ...
                suffix += "@" + std::to_string(
                                    pt == PAWN && !v->promotionZonePawnDrops ?
                                        v->promotionRank :
                                        v->maxRank + 1);
            }
            sync_cout
                << "piece " << v->pieceToChar[pt] << "& "
                << pieceMap.find(pt == KING ? v->kingType : pt)->second->betza
                << suffix << sync_endl;
            PieceType promType = v->promotedPieceType[pt];
            if (promType)
                sync_cout << "piece +" << v->pieceToChar[pt] << "& "
                          << pieceMap.find(promType)->second->betza
                          << sync_endl;
        }
    } else
        sync_cout << "info string variant " << (std::string)o << " files "
                  << v->maxFile + 1 << " ranks " << v->maxRank + 1 << " pocket "
                  << pocketsize << " template " << v->variantTemplate
                  << " startpos " << v->startFen << sync_endl;
}
    ...
    if (CurrentProtocol == XBOARD) {
        for (size_t idx = 0; idx < om.size(); ++idx)
            for (const auto &it : om)
                if (it.second.idx == idx && it.first != "UCI_Variant" &&
                    ...
                    if (o.type == "spin")
                        os << " " << int(stof(o.defaultValue)) << " " << o.min
                           << " " << o.max;
            ...
            for (const auto &it : om)
                if (it.second.idx == idx) {
                    ...
                    if (CurrentProtocol == UCCI || CurrentProtocol == USI) {
                        ...
                        // UCCI skips "name"
                        os << "\noption "
                           << (CurrentProtocol == UCCI ? "" : "name ") << name
                           << " type " << o.type;
                    } else
                        os << "\noption name " << it.first << " type "
                           << o.type;
...
Option::Option(const char *v, const std::vector<std::string> &values,
               OnChange f)
    : type("combo")
    , min(0)
    , max(0)
...
Option &Option::operator=(const string &v)
{
    ...
    if ((type != "button" && v.empty()) ||
        ...
        (type == "combo" && (std::find(comboValues.begin(), comboValues.end(),
                                       v) == comboValues.end())) ||
