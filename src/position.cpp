...
std::ostream &operator<<(std::ostream &os, const Position &pos)
{
    ...
    for (Rank r = pos.max_rank(); r >= RANK_1; --r) {
        for (File f = FILE_A; f <= pos.max_file(); ++f)
            if (pos.unpromoted_piece_on(make_square(f, r)))
                os << " |+"
                   << pos.piece_to_char()[pos.unpromoted_piece_on(
                          make_square(f, r))];
            else
                os << " | "
                   << pos.piece_to_char()[pos.piece_on(make_square(f, r))];
        ...
        if (r == pos.max_rank() || r == RANK_1) {
            ...
            if (c == pos.side_to_move())
                ...
                os << "  ";
            if (!pos.variant()->freeDrops &&
                (pos.piece_drops() || pos.seirawan_gating() ||
                 pos.arrow_gating())) {
                os << " [";
    ...
    if (int(Tablebases::MaxCardinality) >= popcount(pos.pieces()) &&
        ...
        Position p;
        p.set(pos.variant(), pos.fen(), pos.is_chess960(), &st,
              pos.this_thread());
...
void Position::init()
{
    ...
    int count = 0;
    for (Color c : {WHITE, BLACK})
        for (PieceType pt = KNIGHT; pt <= QUEEN || pt == KING;
             pt != QUEEN ? ++pt : pt = KING) {
            Piece pc = make_piece(c, pt);
            for (Square s1 = SQ_A1; s1 <= SQ_MAX; ++s1)
                for (Square s2 = Square(s1 + 1); s2 <= SQ_MAX; ++s2)
                    if ((type_of(pc) != PAWN) &&
                        (attacks_bb(c, type_of(pc), s1, 0) & s2)) {
...
Position &Position::set(const Variant *v, const string &fenStr, bool isChess960,
                        StateInfo *si, Thread *th, bool sfen)
{
    /*
       ...
       by a single letter taken from the standard English names. White pieces
       are designated using upper-case letters ("PNBRQK") whilst Black uses
       lowercase ("pnbrqk"). Blank squares are noted using digits 1 through 8
       ...
       this is recorded only if there is a pawn in position to make an en
       passant capture, and if there really is a pawn that might have advanced
       two squares.
    ...
    */
    ...
    while ((ss >> token) && !isspace(token)) {
        ...
        // Promoted shogi pieces
                 ...
                 (idx = piece_to_char().find(ss.peek())) != string::npos) {
            ss >> token;
            put_piece(make_piece(color_of(Piece(idx)),
    ...
    // 2. Active color
    ss >> token;
    sideToMove = (token != (sfen ? 'w' : 'b') ? WHITE : BLACK); // Invert colors
                                                                // for SFEN
    ss >> token;
    ...
    if (!isdigit(ss.peek()) && !sfen) {
        ...
        // that, in case of Chess960, if an inner rook is associated with the
        // castling right, the castling tag is replaced by the file letter of
        // the involved rook, as for the Shredder-FEN.
        while ((ss >> token) && !isspace(token)) {
            Square rsq;
            ...
            if (token == 'K')
                for (rsq = make_square(FILE_MAX, castling_rank(c));
                     piece_on(rsq) != rook; --rsq) {
                }

            else if (token == 'Q')
                for (rsq = make_square(FILE_A, castling_rank(c));
                     piece_on(rsq) != rook; ++rsq) {
                }
            ...
            if (castling_enabled() && st->castlingKingSquare[c] == SQ_NONE) {
                Bitboard castlingKings = pieces(c, castling_king_piece()) &
                                         rank_bb(castling_rank(c));
                // Ambiguity resolution for 960 variants with more than one
                // "king" e.g., EAH means that an e-file king can castle with a-
                // and h-file rooks
                st->castlingKingSquare[c] =
                    isChess960 && piece_on(rsq) ==
                                      make_piece(c, castling_king_piece()) ?
                        rsq :
                    castlingKings &&
                            (!more_than_one(castlingKings) || isChess960) ?
                        lsb(castlingKings) :
            ...
            if (gating()) {
                ...
                // Do not set castling rights for gates unless there are no
                // pieces in hand, which means that the file is referring to a
                // chess960 castling right.
                else if (!seirawan_gating() ||
                         count_in_hand(c, ALL_PIECES) > 0 || captures_to_hand())
                    continue;
            }
        ...
        if (gating() && castling_enabled())
            for (Color c : {WHITE, BLACK})
                if ((gates(c) & pieces(castling_king_piece())) &&
                    !castling_rights(c) &&
                    (!seirawan_gating() || count_in_hand(c, ALL_PIECES) > 0 ||
                     captures_to_hand())) {
                    Bitboard castling_rooks = gates(c) &
                                              pieces(castling_rook_piece());
        ...
        // Ignore if square is invalid or not on side to move relative rank 6.
            ...
            bool enpassant;
            enpassant = pawn_attacks_bb(~sideToMove, st->epSquare) &
                            pieces(sideToMove, PAWN) &&
                        (pieces(~sideToMove, PAWN) &
                         (st->epSquare + pawn_push(~sideToMove))) &&
                        !(pieces() & (st->epSquare |
                                      (st->epSquare + pawn_push(sideToMove))));
            if (!enpassant)
                st->epSquare = SQ_NONE;
    ...
    if (check_counting()) {
        if (ss >> token && token == '+') {
            ss >> token;
            st->checksRemaining[WHITE] = CheckCount(
                std::max(3 - (token - '0'), 0));
            ss >> token >> token;
            st->checksRemaining[BLACK] = CheckCount(
                std::max(3 - (token - '0'), 0));
        }
    }
...
void Position::set_castling_right(Color c, Square rfrom)
{
    ...
    Square kto = make_square(cr & KING_SIDE ? castling_kingside_file() :
                                              castling_queenside_file(),
                             castling_rank(c));
...
{
    si->blockersForKing[WHITE] = slider_blockers(
        pieces(BLACK), count<KING>(WHITE) ? square<KING>(WHITE) : SQ_NONE,
        si->pinners[BLACK], BLACK);
    si->blockersForKing[BLACK] = slider_blockers(
        pieces(WHITE), count<KING>(BLACK) ? square<KING>(BLACK) : SQ_NONE,
    ...
    for (PieceType pt : piece_types()) {
        si->checkSquares[pt] = ksq != SQ_NONE ?
                                   attacks_bb(~sideToMove, pt, ksq, pieces()) :
                                   Bitboard(0);
        // Collect special piece types that require slower check and evasion
        // detection
        if (AttackRiderTypes[pt] & NON_SLIDING_RIDERS)
            si->nonSlidingRiders |= pieces(pt);
    }
    si->shak = si->checkersBB &
               (byTypeBB[KNIGHT] | byTypeBB[ROOK] | byTypeBB[BERS]);
    si->bikjang = var->bikjangRule && ksq != SQ_NONE ?
                      bool(attacks_bb(sideToMove, ROOK, ksq, pieces()) &
...
void Position::set_state(StateInfo *si) const
{
    ...
    si->checkersBB = count<KING>(sideToMove) ?
                         attackers_to(square<KING>(sideToMove), ~sideToMove) :
                         Bitboard(0);
    si->move = MOVE_NONE;
...
Position &Position::set(const string &code, Color c, StateInfo *si)
{
    ...
    string fenStr = n + "/" + sides[0] +
                    char(FILE_NB - sides[0].length() + '0') + "/" + n + "/" +
                    n + "/" + n + "/" + n + "/" + sides[1] +
                    char(FILE_NB - sides[1].length() + '0') + "/" + n +
                    " w - - 0 10";
...
string Position::fen(bool sfen, bool showPromoted, int countStarted,
                     std::string holdings) const
{
        ...
        for (File f = FILE_A; f <= max_file(); ++f) {
            ...
            if (f <= max_file()) {
                if (unpromoted_piece_on(make_square(f, r)))
                    ...
                    ss << "+"
                       << piece_to_char()[unpromoted_piece_on(
                              make_square(f, r))];
                else {
    ...
    if (!variant()->freeDrops &&
        (piece_drops() || seirawan_gating() || arrow_gating())) {
        ss << '[';
        if (holdings != "-")
            ss << holdings;
        else
    ...
    if (chess960 && can_castle(WHITE_CASTLING) &&
        popcount(pieces(WHITE, castling_king_piece()) &
                 rank_bb(castling_rank(WHITE))) > 1)
    ...
    if (gating() && gates(WHITE) &&
        (!seirawan_gating() || count_in_hand(WHITE, ALL_PIECES) > 0 ||
         captures_to_hand()))
        for (File f = FILE_A; f <= max_file(); ++f)
            if ((gates(WHITE) & file_bb(f))
                ...
                !(can_castle(WHITE_OO) &&
                  f == file_of(castling_rook_square(WHITE_OO))) &&
                !(can_castle(WHITE_OOO) &&
                  f == file_of(castling_rook_square(WHITE_OOO))))
                ss << char('A' + f);

    // Disambiguation for chess960 "king" square
    if (chess960 && can_castle(BLACK_CASTLING) &&
        popcount(pieces(BLACK, castling_king_piece()) &
                 rank_bb(castling_rank(BLACK))) > 1)
    ...
    if (gating() && gates(BLACK) &&
        (!seirawan_gating() || count_in_hand(BLACK, ALL_PIECES) > 0 ||
         captures_to_hand()))
        for (File f = FILE_A; f <= max_file(); ++f)
            if ((gates(BLACK) & file_bb(f))
                ...
                !(can_castle(BLACK_OO) &&
                  f == file_of(castling_rook_square(BLACK_OO))) &&
                !(can_castle(BLACK_OOO) &&
                  f == file_of(castling_rook_square(BLACK_OOO))))
                ss << char('a' + f);

    if (!can_castle(ANY_CASTLING) &&
        !(gating() && (gates(WHITE) | gates(BLACK))))
        ss << '-';
                                   ...
                                   Bitboard &pinners, Color c) const
{
    ...
    if (var->fastAttacks) {
        snipers = ((attacks_bb<ROOK>(s) & pieces(c, QUEEN, ROOK, CHANCELLOR)) |
                   (attacks_bb<BISHOP>(s) &
                    pieces(c, QUEEN, BISHOP, ARCHBISHOP))) &
                  sliders;
        slidingSnipers = snipers;
    } else {
        for (PieceType pt : piece_types()) {
            Bitboard b = sliders &
                         (PseudoAttacks[~c][pt][s] ^ LeaperAttacks[~c][pt][s]) &
                         pieces(c, pt);
            if (b) {
                // Consider asymmetrical moves (e.g., horse)
                if (AttackRiderTypes[pt] & ASYMMETRICAL_RIDERS) {
                    Bitboard asymmetricals = PseudoAttacks[~c][pt][s] &
                                             pieces(c, pt);
                    while (asymmetricals) {
        ...
        if (diagonal_lines() & s) {
            Bitboard diags = diagonal_lines() & PseudoAttacks[~c][BISHOP][s] &
                             sliders & pieces(c, ROOK);
            while (diags) {
    ...
    while (snipers) {
        ...
        bool isHopper = AttackRiderTypes[type_of(piece_on(sniperSq))] &
                        HOPPING_RIDERS;
        Bitboard b = between_bb(s, sniperSq, type_of(piece_on(sniperSq))) &
                     (isHopper ? (pieces() ^ sniperSq) : occupancy);

        if (b && (!more_than_one(b) || (isHopper && popcount(b) == 2))) {
            // Janggi cannons block each other
            if ((pieces(JANGGI_CANNON) & sniperSq) &&
                (pieces(JANGGI_CANNON) & b))
                                ...
                                Bitboard janggiCannons) const
{
    ...
    if (var->fastAttacks) {
        return (pawn_attacks_bb(~c, s) & pieces(c, PAWN)) |
               (attacks_bb<KNIGHT>(s) &
                pieces(c, KNIGHT, ARCHBISHOP, CHANCELLOR)) |
               (attacks_bb<ROOK>(s, occupied) &
                pieces(c, ROOK, QUEEN, CHANCELLOR)) |
               (attacks_bb<BISHOP>(s, occupied) &
                pieces(c, BISHOP, QUEEN, ARCHBISHOP)) |
    ...
    if (var->fastAttacks2) {
        return (pawn_attacks_bb(~c, s) &
               ...
               (LeaperAttacks[~c][SHOGI_PAWN][s] &
                pieces(c, SHOGI_PAWN, SILVER));
    }
    ...
    for (PieceType pt : piece_types())
        if (board_bb(c, pt) & s) {
            ...
            if (AttackRiderTypes[move_pt] & ASYMMETRICAL_RIDERS) {
                Bitboard asymmetricals = PseudoAttacks[~c][move_pt][s] &
                                         pieces(c, pt);
                while (asymmetricals) {
                    Square s2 = pop_lsb(asymmetricals);
    ...
    if (diagonal_lines() & s) {
        ...
        diags |= rider_attacks_bb<RIDER_CANNON_DIAG>(s, occupied) &
                 rider_attacks_bb<RIDER_CANNON_DIAG>(s, occupied &
                                                            ~janggiCannons) &
                 pieces(c, JANGGI_CANNON);
    ...
    // Unpromoted soldiers
    if (b & pieces(SOLDIER) &&
        relative_rank(c, s, max_rank()) < var->soldierPromotionRank)
...
Bitboard Position::attackers_to_pseudo_royals(Color c) const
{
    ...
    while (pseudoRoyals) {
        ...
        if (blast_on_capture() && pseudoRoyalsTheirs & attacks_bb<KING>(sr))
            // skip if capturing this piece would blast all of the attacker's
            // pseudo-royal pieces
            continue;
...
bool Position::legal(Move m) const
{
    ...
    assert(!count<KING>(us) ||
           piece_on(square<KING>(us)) == make_piece(us, KING));
    assert(board_bb() & to);

    // Illegal checks
    if ((!checking_permitted() ||
         (sittuyin_promotion() && type_of(m) == PROMOTION) ||
    ...
    if (must_drop() && type_of(m) != DROP &&
        count_in_hand(us, var->mustDropType) > 0) {
        if (checkers()) {
    ...
    if (drop_opposite_colored_bishop() && type_of(m) == DROP) {
        if (type_of(moved_piece(m)) != BISHOP) {
            Bitboard remaining = drop_region(us, BISHOP) & ~pieces() &
                                 ~square_bb(to);
            ...
            if (popcount((DarkSquares & to ? DarkSquares : ~DarkSquares) &
                         pieces(us, BISHOP)) +
                    1 >
    ...
    if (var->extinctionPseudoRoyal) {
        ...
        if (type_of(m) == CASTLING) {
            ...
            kto = make_square(to > from ? castling_kingside_file() :
                                          castling_queenside_file(),
                              castling_rank(us));
            ...
            if (st->pseudoRoyals & from)
                for (Square s = from; s != kto; s += step)
                    if (!(blast_on_capture() &&
                          (attacks_bb<KING>(s) & st->pseudoRoyals &
                           pieces(~sideToMove))) &&
        ...
        if (capture(m) && blast_on_capture())
            occupied &= ~((attacks_bb<KING>(kto) & (pieces() ^ pieces(PAWN))) |
                          kto);
        ...
        if (type_of(m) == PROMOTION &&
            extinction_piece_types().find(promotion_type(m)) !=
    ...
    if (type_of(m) == CASTLING) {
        // After castling, the rook and king final positions are the same in
        // Chess960 as they would be in standard chess.
        to = make_square(to > from ? castling_kingside_file() :
                                     castling_queenside_file(),
                         castling_rank(us));
        ...
        for (Square s = to; s != from; s += step)
            if (attackers_to(s, ~us) ||
                (var->flyingGeneral &&
                 (attacks_bb(~us, ROOK, s, pieces() ^ from) &
                  pieces(~us, KING))))
                return false;
    ...
    if ((var->flyingGeneral && count<KING>(us)) || st->bikjang) {
        ...
        if (attacks_bb(~us, ROOK, s, occupied) & pieces(~us, KING) &
            ~square_bb(to))
            return false;
    }

    // Makpong rule
    if (var->makpongRule && checkers() && type_of(moved_piece(m)) == KING &&
        (checkers() ^ to))
        return false;
    ...
    if (type_of(moved_piece(m)) == JANGGI_CANNON)
        janggiCannons = (type_of(m) == DROP ? janggiCannons :
                                              janggiCannons ^ from) |
                        to;
...
bool Position::pseudo_legal(const Move m) const
{
    ...
    if (type_of(m) == DROP)
        return piece_drops() && pc != NO_PIECE && color_of(pc) == us &&
               ...
               (type_of(pc) == in_hand_piece_type(m) ||
                (drop_promoted() &&
                 type_of(pc) == promoted_piece_type(in_hand_piece_type(m))));
    ...
    if (type_of(pc) == JANGGI_CANNON &&
        (pieces(JANGGI_CANNON) & (between_bb(from, to) | to)))
        return false;
    ...
    if (checkers() && !(checkers() & non_sliding_riders())) {
        if (type_of(pc) != KING) {
            ...
            // Our move must be a blocking evasion or a capture of the checking
            // piece
            Square checksq = lsb(checkers());
            if (!(between_bb(square<KING>(us), lsb(checkers())) & to) ||
                ((LeaperAttacks[~us][type_of(piece_on(checksq))][checksq] &
                  square<KING>(us)) &&
                 !(checkers() & to)))
                return false;
        }
        // In case of king moves under check we have to remove king so as to
        // catch invalid moves like b1a1 when opposite queen is on c1.
...
bool Position::gives_check(Move m) const
{
    ...
    if (type_of(m) != PROMOTION && type_of(m) != PIECE_PROMOTION &&
        type_of(m) != PIECE_DEMOTION && type_of(m) != CASTLING) {
        PieceType pt = type_of(moved_piece(m));
        if (AttackRiderTypes[pt] & (HOPPING_RIDERS | ASYMMETRICAL_RIDERS)) {
            Bitboard occupied = (type_of(m) != DROP ? pieces() ^ from :
                                                      pieces()) |
                                to;
            if (attacks_bb(sideToMove, pt, to, occupied) &
    ...
    if (((type_of(m) != DROP && (blockers_for_king(~sideToMove) & from)) ||
        ...
        attackers_to(square<KING>(~sideToMove),
                     (type_of(m) == DROP ? pieces() : pieces() ^ from) | to,
                     sideToMove, janggiCannons))
        return true;

    // Is there a check by gated pieces?
    if (is_gating(m) && attacks_bb(sideToMove, gating_type(m), gating_square(m),
                                   (pieces() ^ from) | to) &
                            square<KING>(~sideToMove))
        return true;
    ...
    if (more_than_one(diagonal_lines() & (to | square<KING>(~sideToMove)))) {
        ...
        if (diagType && (attacks_bb(sideToMove, diagType, to, occupied) &
                         square<KING>(~sideToMove)))
            return true;
        else if (pt == JANGGI_CANNON &&
    ...
    case PIECE_PROMOTION:
        return attacks_bb(sideToMove,
                          promoted_piece_type(type_of(moved_piece(m))), to,
                          pieces() ^ from) &
               square<KING>(~sideToMove);

    case PIECE_DEMOTION:
        return attacks_bb(sideToMove, type_of(unpromoted_piece_on(from)), to,
                          pieces() ^ from) &
               square<KING>(~sideToMove);
    ...
    case EN_PASSANT: {
        ...
        return attackers_to(square<KING>(~sideToMove), b) & pieces(sideToMove) &
               b;
    }
    default: // CASTLING
    {
        ...
        Square kfrom = from;
        Square rfrom = to;
        Square kto = make_square(rfrom > kfrom ? castling_kingside_file() :
                                                 castling_queenside_file(),
        ...
        if (castling_rank(WHITE) > RANK_1 &&
            ((blockers_for_king(~sideToMove) & rfrom) ||
             (non_sliding_riders() & pieces(sideToMove))) &&
            attackers_to(square<KING>(~sideToMove),
                         (pieces() ^ kfrom ^ rfrom) | rto | kto, sideToMove))
            return true;
...
void Position::do_move(Move m, StateInfo &newSt, bool givesCheck)
{
    ...
    std::memcpy(static_cast<void *>(&newSt), static_cast<void *>(st),
    ...
    if (captured) {
        ...
        if (type_of(captured) == PAWN) {
            if (type_of(m) == EN_PASSANT) {
                ...
                assert((var->enPassantRegion & to) &&
                       relative_rank(~us, to, max_rank()) <=
                           Rank(double_step_rank_max() + 1) &&
                       relative_rank(~us, to, max_rank()) >
                           double_step_rank_min());
                assert(piece_on(to) == NO_PIECE);
        ...
        if (captures_to_hand()) {
            ...
            k ^= Zobrist::inHand[pieceToHand]
                                [pieceCountInHand[color_of(pieceToHand)]
                                                 [type_of(pieceToHand)] -
                                 1] ^
                 Zobrist::inHand[pieceToHand][pieceCountInHand[color_of(
                     pieceToHand)][type_of(pieceToHand)]];

            if (Eval::useNNUE) {
                dp.handPiece[1] = pieceToHand;
                dp.handCount[1] = pieceCountInHand[color_of(pieceToHand)]
                                                  [type_of(pieceToHand)];
            }
    ...
    if (type_of(m) == DROP) {
        ...
        k ^= Zobrist::psq[pc][to] ^
             Zobrist::inHand[pc_hand][pieceCountInHand[color_of(pc_hand)]
                                                      [type_of(pc_hand)] -
                                      1] ^
             Zobrist::inHand[pc_hand][pieceCountInHand[color_of(pc_hand)]
                                                      [type_of(pc_hand)]];
    ...
    if (flip_enclosed_pieces() && !is_pass(m)) {
        ...
        while (to_flip) {
            ...
            st->materialKey ^=
                Zobrist::psq[resulting][pieceCount[resulting] - 1];
    ...
    if (type_of(m) == DROP) {
        ...
        if (castling_dropped_piece() && rank_of(to) == castling_rank(us)) {
            if (type_of(pc) == castling_king_piece() &&
                file_of(to) == castling_king_file()) {
                ...
                if ((file_of(to) == FILE_A || file_of(to) == max_file()) &&
                    ...
                    st->castlingKingSquare[us] = make_square(
                        castling_king_file(), castling_rank(us));
    ...
    if (type_of(pc) == PAWN) {
        ...
        if (type_of(m) != DROP && std::abs(int(to) - int(from)) == 2 * NORTH &&
            ...
            Piece promotion = make_piece(us, type_of(m) == PROMOTION ?
                                                 promotion_type(m) :
            ...
            remove_piece(to);
            put_piece(promotion, to, true,
                      type_of(m) == PIECE_PROMOTION ? pc : NO_PIECE);
            ...
            st->materialKey ^=
                Zobrist::psq[promotion][pieceCount[promotion] - 1] ^
        ...
        st->pawnKey ^= (type_of(m) != DROP ? Zobrist::psq[pc][from] : 0) ^
                       Zobrist::psq[pc][to];
        ...
        // Update material
        st->nonPawnMaterial[us] += PieceValue[MG][demotion] -
                                   PieceValue[MG][pc];
    }
    ...
    if (gating()) {
        ...
        if (gates(them) & to)
            st->gatesBB[them] ^= to;
        if (seirawan_gating() && count_in_hand(us, ALL_PIECES) == 0 &&
            !captures_to_hand())
            st->gatesBB[us] = 0;
    }

    // Remove king leaping right when aimed by a rook
    if (cambodian_moves() && type_of(pc) == ROOK &&
        (square<KING>(them) & gates(them) & attacks_bb<ROOK>(to)))
    ...
    if (captured && blast_on_capture()) {
        ...
        Bitboard blast = (attacks_bb<KING>(to) & (pieces() ^ pieces(PAWN))) |
                         to;
        while (blast) {
            ...
            // Update board and piece lists
            // In order to not have to store the values of both board and
            // unpromotedBoard, demote promoted pieces, but keep promoted pawns
            // as promoted, and store demotion/promotion bitboards to
            // disambiguate the piece state
            bool capturedPromoted = is_promoted(bsq);
            Piece unpromotedCaptured = unpromoted_piece_on(bsq);
            st->unpromotedBycatch[bsq] = unpromotedCaptured ?
                                             unpromotedCaptured :
                                             bpc;
            ...
            if (captures_to_hand()) {
                ...
                if (Eval::useNNUE) {
                    ...
                    dp.handCount[dp.dirty_num - 1] = pieceCountInHand[color_of(
                        pieceToHand)][type_of(pieceToHand)];
                }
            }
    ...
    st->checkersBB = givesCheck ?
                         attackers_to(square<KING>(them), us) & pieces(us) :
                         Bitboard(0);

    sideToMove = ~sideToMove;

    if (counting_rule()) {
        if (counting_rule() != ASEAN_COUNTING && type_of(captured) == PAWN &&
            count<ALL_PIECES>(~sideToMove) == 1 && !count<PAWN>() &&
            count_limit(~sideToMove)) {
            st->countingLimit = 2 * count_limit(~sideToMove);
            st->countingPly = 2 * count<ALL_PIECES>() - 1;
        }

        if ((!st->countingLimit || ((captured || type_of(m) == PROMOTION) &&
                                    count<ALL_PIECES>(sideToMove) == 1)) &&
            count_limit(sideToMove)) {
            st->countingLimit = 2 * count_limit(sideToMove);
            st->countingPly = counting_rule() == ASEAN_COUNTING ||
                                      count<ALL_PIECES>(sideToMove) > 1 ?
                                  0 :
                                  2 * count<ALL_PIECES>();
        }
...
void Position::undo_move(Move m)
{
    ...
    assert(type_of(m) == DROP || empty(from) || type_of(m) == CASTLING ||
           is_gating(m) || (type_of(m) == PROMOTION && sittuyin_promotion()) ||
           (is_pass(m) && pass()));
    ...
    if (st->capturedPiece && blast_on_capture()) {
        ...
        while (blast) {
            ...
            if (bpc) {
                ...
                if (captures_to_hand())
                    remove_from_hand(
                        !drop_loop() && (st->promotedBycatch & bsq) ?
    ...
    if (type_of(m) == CASTLING) {
        ...
        if (type_of(m) == DROP)
            undrop_piece(make_piece(us, in_hand_piece_type(m)), to); // Remove
                                                                     // the
                                                                     // dropped
                                                                     // piece
        else
        ...
        if (st->capturedPiece) {
            ...
            if (type_of(m) == EN_PASSANT) {
                ...
                assert(relative_rank(~us, to, max_rank()) <=
                       Rank(double_step_rank_max() + 1));
            ...
            put_piece(st->capturedPiece, capsq, st->capturedpromoted,
                      st->unpromotedCapturedPiece); // Restore the captured
                                                    // piece
            if (captures_to_hand())
                remove_from_hand(
                    !drop_loop() && st->capturedpromoted ?
                           ...
                           Square &rto)
{
    ...
    to = make_square(kingSide ? castling_kingside_file() :
                                castling_queenside_file(),
                     castling_rank(us));
    ...
    board[Do ? from : to] = board[Do ? rfrom : rto] = NO_PIECE; // Since
                                                                // remove_piece
                                                                // doesn't do it
                                                                // for us
    ...
    if (captured) {
        ...
        if (captures_to_hand()) {
            Piece removeFromHand = !drop_loop() && is_promoted(to) ?
                                       make_piece(~color_of(captured), PAWN) :
                                       ~captured;
            k ^= Zobrist::inHand[removeFromHand]
                                [pieceCountInHand[color_of(removeFromHand)]
                                                 [type_of(removeFromHand)] +
                                 1] ^
                 Zobrist::inHand[removeFromHand][pieceCountInHand[color_of(
                     removeFromHand)][type_of(removeFromHand)]];
        }
    }
    if (type_of(m) == DROP) {
...
Value Position::blast_see(Move m) const
{
    ...
    Bitboard blast = ((attacks_bb<KING>(to) & ~pieces(PAWN)) | fromto) &
                     pieces();
    ...
    if (!capture(m)) {
        ...
        while (attackers) {
            ...
            if (extinction_piece_types().find(type_of(piece_on(s))) ==
                extinction_piece_types().end())
                minAttacker = std::min(minAttacker,
                                       blast & s ?
                                           VALUE_ZERO :
                                           CapturePieceValue[MG][piece_on(s)]);
        }
    ...
    while (blast) {
        ...
        if (extinction_piece_types().find(type_of(bpc)) !=
            extinction_piece_types().end())
            return color_of(bpc) == us ? extinction_value() :
                   capture(m)          ? -extinction_value() :
                                         VALUE_ZERO;
        result += color_of(bpc) == us ? -CapturePieceValue[MG][bpc] :
                                        CapturePieceValue[MG][bpc];
    }

    return capture(m) || must_capture() ? result - 1 :
                                          std::min(result, VALUE_ZERO);
}
...
bool Position::see_ge(Move m, Value threshold) const
{
    ...
    // nCheck
    if (check_counting() && color_of(moved_piece(m)) == sideToMove &&
        gives_check(m))
        return true;
    ...
    if (extinction_value() != VALUE_NONE && piece_on(to) &&
        ((extinction_piece_types().find(type_of(piece_on(to))) !=
         ...
         (extinction_piece_types().find(ALL_PIECES) !=
              extinction_piece_types().end() &&
    ...
    if (var->flyingGeneral) {
        if (attackers & pieces(stm, KING))
            attackers |= attacks_bb(stm, ROOK, to, occupied & ~pieces(ROOK)) &
                         pieces(~stm, KING);
        if (attackers & pieces(~stm, KING))
            attackers |= attacks_bb(~stm, ROOK, to, occupied & ~pieces(ROOK)) &
                         pieces(stm, KING);
...
/// Position::is_optinal_game_end() tests whether the position may end the game
/// by 50-move rule, by repetition, or a variant rule that allows a player to
/// claim a game result.

bool Position::is_optional_game_end(Value &result, int ply,
                                    int countStarted) const
{
    // n-move rule
    if (n_move_rule() && st->rule50 > (2 * n_move_rule() - 1) &&
        ...
        if (var->chasingRule == AXF_CHASING && st->pliesFromNull >= 20) {
            ...
            offset = 2 * std::max(std::max(checkThem, checkUs) - 10, 0) +
                     20 * (CurrentProtocol == UCCI ||
                           CurrentProtocol == UCI_CYCLONE);
        }
        if (st->rule50 - offset > (2 * n_move_rule() - 1)) {
            result = var->materialCounting ?
                         convert_mate_value(material_counting_result(), ply) :
                         VALUE_DRAW;
            return true;
        }
    }
    ...
    if (n_fold_rule()) {
        ...
        if (end >= 4) {
            ...
            bool perpetualUs = st->previous->checkersBB &&
                               stp->previous->checkersBB;
            Bitboard chaseThem = undo_move_board(st->chased,
                                                 st->previous->move) &
                                 stp->chased;
            Bitboard chaseUs = undo_move_board(st->previous->chased,
                                               stp->move) &
                               stp->previous->chased;
            int moveRepetition = var->moveRepetitionIllegal &&
                                         type_of(st->move) == NORMAL &&
                                         !st->previous->checkersBB &&
                                         !stp->previous->checkersBB &&
                                         (board_bb(~side_to_move(),
                                                   type_of(piece_on(
                                                       to_sq(st->move)))) &
            ...
            for (int i = 4; i <= end; i += 2) {
                ...
                if (moveRepetition > 0) {
                    if (i + 1 <= end &&
                        stp->previous->previous->previous->checkersBB)
                        moveRepetition = 0;
                    else if (moveRepetition < 4) {
                        if (stp->previous->previous->move ==
                            reverse_move(
                                (moveRepetition == 1 ? st : stp)->move))
                            moveRepetition++;
                        else
                            moveRepetition = 0;
                    } else {
                        ...
                        if (!stp->previous->previous->capturedPiece &&
                            from_sq(stp->move) ==
                                to_sq(stp->previous->previous->move)) {
                            result = VALUE_MATE;
                            return true;
                        } else
                            moveRepetition = 0;
                ...
                if (i != st->pliesFromNull)
                    chaseThem = undo_move_board(chaseThem,
                                                stp->previous->move) &
                                stp->previous->previous->chased;
                stp = stp->previous->previous;
                ...
                // root.
                if (stp->key == st->key &&
                    ++cnt + 1 == (ply > i && !var->moveRepetitionIllegal ?
                                      2 :
                                      n_fold_rule())) {
                    result = convert_mate_value(
                        var->perpetualCheckIllegal &&
                                (perpetualThem || perpetualUs) ?
                            (!perpetualUs   ? VALUE_MATE :
                             !perpetualThem ? -VALUE_MATE :
                                              VALUE_DRAW) :
                        var->chasingRule && (chaseThem || chaseUs) ?
                            (!chaseUs   ? VALUE_MATE :
                             !chaseThem ? -VALUE_MATE :
                                          VALUE_DRAW) :
                        var->nFoldValueAbsolute && sideToMove == BLACK ?
                            -var->nFoldValue :
                            var->nFoldValue,
                        ply);
                    if (result == VALUE_DRAW && var->materialCounting)
                        result = convert_mate_value(material_counting_result(),
                                                    ply);
                    return true;
                }
                ...
                if (i + 1 <= end) {
                    ...
                    chaseUs = undo_move_board(chaseUs, stp->move) &
                              stp->previous->chased;
                }
            }
        }
...
{
    // Extinction
    // Extinction does not apply for pseudo-royal pieces, because they can not
    // be captured
    if (extinction_value() != VALUE_NONE &&
        (!var->extinctionPseudoRoyal || blast_on_capture())) {
        for (Color c : {~sideToMove, sideToMove})
            for (PieceType pt : extinction_piece_types())
                if (count_with_hand(c, pt) <= var->extinctionPieceCount &&
                    count_with_hand(~c, pt) >=
                        var->extinctionOpponentPieceCount +
                            (extinction_claim() && c == sideToMove)) {
                    result = c == sideToMove ? extinction_value(ply) :
                                               -extinction_value(ply);
                    return true;
                }
    }
    // capture the flag
    if (capture_the_flag_piece() && flag_move() &&
        (capture_the_flag(sideToMove) &
         pieces(sideToMove, capture_the_flag_piece()))) {
        result = (capture_the_flag(~sideToMove) &
                  pieces(~sideToMove, capture_the_flag_piece())) &&
                         sideToMove == WHITE ?
                     VALUE_DRAW :
                     mate_in(ply);
    ...
    if (capture_the_flag_piece() &&
        ...
        if (flag_move() && sideToMove == BLACK && !checkers() &&
            count<KING>(sideToMove) &&
            (capture_the_flag(sideToMove) &
    ...
    if (st->pliesFromNull > 0 && ((st->bikjang && st->previous->bikjang) ||
    ...
    if (tsumeMode && !count<KING>(~sideToMove) && count<KING>(sideToMove) &&
        !checkers()) {
        result = mate_in(ply);
        return true;
    }
...
Bitboard Position::chased() const
{
    ...
    if (var->flyingGeneral) {
        Bitboard kingFilePieces = file_bb(file_of(square<KING>(~sideToMove))) &
                                  pieces(sideToMove);
        if ((kingFilePieces & pieces(sideToMove, KING)) &&
            !more_than_one(kingFilePieces & ~pieces(KING)))
            pins |= kingFilePieces & ~pieces(KING);
    }
    auto addChased = [&](Square attackerSq, PieceType attackerType,
                         Bitboard attacks) {
        if (attacks & ~b) {
            // Exclude attacks on unpromoted soldiers and checks
            attacks &= ~(pieces(sideToMove, KING, SOLDIER) ^
                         promoted_soldiers(sideToMove));
            ...
            // - pins
            if (attackerType == HORSE &&
                (PseudoAttacks[WHITE][FERS][attackerSq] & pieces())) {
                Bitboard horses = attacks & pieces(sideToMove, attackerType);
                while (horses) {
                    Square s = pop_lsb(horses);
                    if (attacks_bb(sideToMove, attackerType, s, pieces()) &
                        attackerSq)
                        attacks ^= s;
                }
            } else
            ...
            while (attacks) {
                ...
                Bitboard roots = attackers_to(s, pieces() ^ attackerSq,
                                              sideToMove) &
                                 ~pins;
                if (!roots ||
                    (var->flyingGeneral && roots == pieces(sideToMove, KING) &&
                     (attacks_bb(sideToMove, ROOK, square<KING>(~sideToMove),
                                 pieces() ^ attackerSq) &
                      s)))
                    b |= s;
            }
    ...
    if (movedPiece != KING && movedPiece != SOLDIER) {
        Bitboard directAttacks = attacks_from(~sideToMove, movedPiece, to) &
                                 pieces(sideToMove);
        // Only new attacks count. This avoids expensive comparison of previous
        // and new attacks.
    ...
    // Discovered attacks
    Bitboard discoveryCandidates =
        (PseudoAttacks[WHITE][WAZIR][from] & pieces(~sideToMove, HORSE)) |
    ...
    while (discoveryCandidates) {
        ...
        Bitboard discoveries =
            pieces(sideToMove) &
            attacks_bb(~sideToMove, discoveryPiece, s, pieces()) &
            ~attacks_bb(~sideToMove, discoveryPiece, s,
                        (captured_piece() ? pieces() : pieces() ^ to) ^ from);
    ...
    if (st->pliesFromNull > 0) {
        // Fake roots
        Bitboard newPins = st->blockersForKing[sideToMove] &
                           ~st->previous->blockersForKing[sideToMove] &
                           pieces(sideToMove);
        while (newPins) {
            Square s = pop_lsb(newPins);
            PieceType pinnedPiece = type_of(piece_on(s));
            Bitboard fakeRooted = pieces(sideToMove) &
                                  ~(pieces(sideToMove, KING, SOLDIER) ^
                                    promoted_soldiers(sideToMove)) &
                                  attacks_bb(sideToMove, pinnedPiece, s,
                                             pieces());
            while (fakeRooted) {
                Square s2 = pop_lsb(fakeRooted);
                if (attackers_to(s2, ~sideToMove) &
                    ~blockers_for_king(~sideToMove))
                    b |= s2;
            }
        }
        // Discovered checks
        Bitboard newDiscoverers = st->blockersForKing[sideToMove] &
                                  ~st->previous->blockersForKing[sideToMove] &
                                  pieces(~sideToMove);
        while (newDiscoverers) {
            Square s = pop_lsb(newDiscoverers);
            PieceType discoveryPiece = type_of(piece_on(s));
            Bitboard discoveryAttacks = attacks_from(~sideToMove,
                                                     discoveryPiece, s) &
                                        pieces(sideToMove);
            // Include all captures except where the king can pseudo-legally
            // recapture
            b |= discoveryAttacks &
                 ~attacks_from(sideToMove, KING, square<KING>(sideToMove));
            // Include captures where king can not legally recapture
            discoveryAttacks &= attacks_from(sideToMove, KING,
                                             square<KING>(sideToMove));
            while (discoveryAttacks) {
                Square s2 = pop_lsb(discoveryAttacks);
                if (attackers_to(s2, pieces() ^ s ^ square<KING>(sideToMove),
                                 ~sideToMove) &
                    ~square_bb(s))
                    b |= s2;
            }
...
bool Position::has_game_cycle(int ply) const
{
    ...
    if (end < 3 || var->nFoldValue != VALUE_DRAW ||
        var->perpetualCheckIllegal || var->materialCounting ||
    ...
    for (int i = 3; i <= end; i += 2) {
        ...
        if ((j = H1(moveKey), cuckoo[j] == moveKey) ||
            ...
            if (!((between_bb(s1, s2) ^ s2) & pieces())) {
                ...
                // the same location, so we have to select which square to
                // check.
...
int Position::count_limit(Color sideToCount) const
{
    ...
    case CAMBODIAN_COUNTING:
        // No counting for side to move
        if (count<ALL_PIECES>(sideToCount) > 3 ||
            count<ALL_PIECES>(~sideToCount) == 1)
            return 0;
        // Board's honor rule
        if (count<ALL_PIECES>(sideToCount) > 1)
            return 63;
...
bool Position::pos_is_ok() const
{
    ...
    if ((sideToMove != WHITE && sideToMove != BLACK) ||
        ...
        (ep_square() != SQ_NONE &&
         relative_rank(~sideToMove, ep_square(), max_rank()) >
             Rank(double_step_rank_max() + 1)))
    ...
    for (Color c : {WHITE, BLACK})
        for (CastlingRights cr : {c & KING_SIDE, c & QUEEN_SIDE}) {
            ...
            if (piece_on(castlingRookSquare[cr]) !=
                    make_piece(c, castling_rook_piece()) ||
