...
{
    fast = sitRequested = partnerDead = weDead = weWin = weVirtualWin =
        weVirtualLoss = false;
    time = opptime = 0;
}
...
{
    if (p == ALL_PARTNERS || (p == FAIRY && isFairy) ||
        (p == HUMAN && !isFairy))
...
void PartnerHandler::parse_partner(std::istringstream &is)
{
    ...
    if (is >> token)
        ...
        ptell("partner Fairy-Stockfish is an engine. Ask it 'help' for "
              "supported commands.");
    else
        isFairy = false;
}
...
void PartnerHandler::parse_ptell(std::istringstream &is, const Position &pos)
{
    ...
    if (token == "partner") {
        ...
        if (!(is >> token)) {
            ptell<HUMAN>("I listen to the commands help, sit, go, move, fast, "
                         "slow, dead, x, time, and otim.");
            ptell<HUMAN>("Tell 'help sit', etc. for details.");
        } else if (token == "sit")
            ptell<HUMAN>("After receiving 'sit', I stop moving. Also see "
                         "'go'.");
        else if (token == "go")
            ptell<HUMAN>("After receiving 'go', I will no longer sit.");
        else if (token == "move") {
            ptell<HUMAN>("After receiving 'move', I will move immediately.");
            ptell<HUMAN>("If you specify a valid move, e.g., 'move e2e4', I "
                         "will play it.");
        } else if (token == "fast")
            ptell<HUMAN>("After receiving 'go', I will play fast.");
        else if (token == "slow")
            ptell<HUMAN>("After receiving 'slow', I will play at normal "
                         "speed.");
        else if (token == "dead")
            ptell<HUMAN>("After receiving 'dead', I assume you are dead and I "
                         "play fast.");
        else if (token == "x")
            ptell<HUMAN>("After receiving 'x', I assume I can play normally "
                         "again.");
        else if (token == "time") {
            ptell<HUMAN>("'time' together with your time in centiseconds "
                         "allows me to consider your time.");
            ptell<HUMAN>("E.g., 'time 1000' for 10 seconds.");
        } else if (token == "otim")
            ptell<HUMAN>("'otim' together with your opponent's time in "
                         "centiseconds allows me to consider his time.");
        ...
        if (is >> token) {
            // if the given move is valid and we can still abort the search,
            // play it
